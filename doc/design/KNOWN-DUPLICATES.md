...
- Chat composers: the shared `ChatComposer` vs `MarkdownEditor`-based issue composer — deliberately NOT unified in a prior pass (PAP-101); audit the overlap but flag as "Needs human decision".
...
- Status glyphs/chips: `StatusIcon` vs inline-mention chips vs issue chips — intentionally separate systems per prior work; document, don't merge.
