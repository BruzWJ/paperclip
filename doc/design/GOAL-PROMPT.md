...
1. The Storybook visual snapshot suite passes against a Phase 0
   ...
   Baseline scope: primitives in apps/ui/src/components/ui/
   (add minimal stories only for those) plus the existing stories in
   apps/ui/storybook/stories/. No stories for the ~277 feature components.
2. apps/ui/src/index.css (plus any tokens.css it imports) is the only token
...
3. rg gates pass: zero hex color literals, zero arbitrary px/bracket
   ...
   apps/ui/src/components/** and apps/ui/src/pages/** outside the documented
...
- No redesign, no layout changes, no new colors/typefaces, no component
  ...
  (task vocabulary is a separate later run), no new dependencies beyond
...
- Set up Storybook visual snapshot testing (Storybook test-runner with image snapshots, or equivalent already-compatible tooling; Storybook lives at `apps/ui/storybook/`, launched via `pnpm storybook`).
- Coverage scope: the shared primitives in `apps/ui/src/components/ui/` (add a minimal story for any of the ~24 that lack one) plus all existing stories under `apps/ui/storybook/stories/`. Do NOT write stories for the ~277 feature components in this run.
...
- Produce `TOKEN-AUDIT.md` at the repo root: every hardcoded color/spacing/radius/type/shadow value in `apps/ui/src/`, its frequency, file locations, and near-duplicate clusters (e.g. 13/14/15px used interchangeably). Flag clusters for human review — do NOT merge them. Cross-reference the ~80 existing tokens in `apps/ui/src/index.css`: for each hardcoded value, note whether it exactly matches an existing token.
...
- Token destination is `apps/ui/src/index.css` (Tailwind v4; optionally a `tokens.css` imported by index.css). Do NOT create a parallel token source. Tokens that must be runtime-tunable go in a NON-inline block (`@theme inline` bakes literals).
