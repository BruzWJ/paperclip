...
- [apps/server/src/routes/plugins.ts](../../apps/server/src/routes/plugins.ts#L1898) saved one global plugin config row, then wrote bindings into `company_secret_bindings` grouped by each referenced secret's owning company.
- [packages/db/schema/plugin_config.ts](../../packages/db/schema/plugin_config.ts#L15) stored one config row per plugin, with no company dimension.
- [packages/db/schema/company_secret_bindings.ts](../../packages/db/schema/company_secret_bindings.ts#L5) already modeled bindings as company-scoped.
- [apps/server/src/services/plugin-secrets-handler.ts](../../apps/server/src/services/plugin-secrets-handler.ts#L212) resolved by `pluginId` + secret UUID, with no active company context from the bridge call.
- [packages/plugins/sdk/src/worker-rpc-host.ts](../../packages/plugins/sdk/src/worker-rpc-host.ts#L384) exposed `ctx.config.get()` and `ctx.secrets.resolve()` without a company parameter.
