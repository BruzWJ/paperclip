...
- End onboarding with a **real first issue**, not a blank dashboard.
...
- generate a local `onboarding.txt` / LLM transfer prompt
...
- create starter objects:
  ...
  - first suggested issue

### Backend / API

- Add `GET /api/onboarding/recommendation`
- Add `GET /api/onboarding/llm-transfer.txt`
...
There is a real tension here: the transcript says users want “chat with my CEO,” while the public product definition says Paperclip is **not a chatbot** and V1 communication is **issues + comments only**. At the same time, the repo is already exploring plugin infrastructure and even a chat plugin via plugin SSE streaming. The clean resolution is: **make the core surface conversational, but keep the data model issue/thread-centric; reserve full chat as an optional plugin**. ([GitHub][2])
...
```ts
type ComposerMode = "ask" | "issue" | "decision";
...
- On agent page: “Tell the designer to make this cleaner” → appends an instruction comment to an issue or spawns a new delegated issue.
- On approval page: “Why are you asking to hire?” → appends a board comment to the approval context.

Add issue kinds:

```ts
type IssueKind = "work" | "strategy" | "question" | "decision";
```
...
- From that thread, the user can create/approve issues with one click.
...
- First issue / first hire / first completion moments are visibly celebrated.
...
## 9) Project workspaces, previews, and PR transfer — without becoming GitHub
...
Paperclip should manage the **issue → workspace → preview/PR → review transfer** lifecycle, but leave diffs/review/merge to external tools.
