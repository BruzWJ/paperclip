...
- **Align to goals** — task and project hierarchy keeps work tied to company intent
- **Preserve auditable task context** — comments, documents, work products, attachments, and the task Session stay attached to that task
...
- Canonical task ownership, lifecycle, and typed creator authority
- Budget and token spend tracking
- Task comments, documents, work products, attachments, and company state
- Goal, project, and task/sub-task hierarchy
- Task-execution monitoring — know which authorized runs are queued, active, terminal, or stuck

It also enforces execution-control semantics such as one canonical owner,
monotonic ownership epochs, persisted task-execution refs, immutable
execution views, ordinary blockers, typed recovery/system escalation, and
task/epoch run-directory bindings.

### 2. Execution Worker and ACP Agents

Paperclip's worker resolves the task's server-owned run directory and invokes ACPX's public,
...
attempt and retains only task execution, request-scoped capabilities, and the
