...
Router plugin's test-file ignore pattern. Keep Paperclip domain UI beside its
route consumer in a file or directory whose basename starts with `-`, so the
Router plugin ignores it. When multiple route branches share a helper, place it
at their closest common route ancestor under the same rule. Reserve
`apps/ui/src/components/` for neutral, reusable UI and registry sources. There
is no parallel `src/features/` or `src/pages/` tree, and this Vite application
does not use Next.js App Router conventions.
