...
- **Sub-tasks as a workflow checklist** — Ordered sub-tasks now render as a dependency-aware progress checklist with current-step affordances, blocker context, and de-emphasized done items, replacing the flat table presentation. ([#4523](https://github.com/paperclipai/paperclip/pull/4523), [#4588](https://github.com/paperclipai/paperclip/pull/4588))
- **Task subtree pause, cancel, and restore** — Board operators can now hold, cancel, or restore a whole task subtree from the task detail. Holds are durable, dependency-aware, and keep descendant execution and wake behavior coherent. ([#4332](https://github.com/paperclipai/paperclip/pull/4332))
- **First-class task references** — Mentions of `PAP-123`-style task IDs in task text, comments, and documents now persist as durable reference relationships. The board surfaces backlinks, queryable cross-task context, and an idempotent backfill for existing content. ([#4214](https://github.com/paperclipai/paperclip/pull/4214))
...
- **Heartbeat scheduling is blocker-aware** — The scheduler now respects `blockedByTaskIds` graphs and avoids waking dependents until their blockers resolve. ([#4157](https://github.com/paperclipai/paperclip/pull/4157))
- **Cancel stale queued and scheduled work on graph changes** — Queued heartbeats are gated at claim time and cancelled when the task assignee, status, or review participant moves on. Scheduled retries are cancelled on ownership changes or explicit task cancellation. ([#4445](https://github.com/paperclipai/paperclip/pull/4445), [#4534](https://github.com/paperclipai/paperclip/pull/4534), [#4234](https://github.com/paperclipai/paperclip/pull/4234))
...
- **Task composer and document polish** — Long document display, task composer affordances, board workflow polish, and refined markdown task-reference rendering. ([#4224](https://github.com/paperclipai/paperclip/pull/4224), [#4382](https://github.com/paperclipai/paperclip/pull/4382), [#4420](https://github.com/paperclipai/paperclip/pull/4420), [#4447](https://github.com/paperclipai/paperclip/pull/4447))
...
- **Normalized escaped multiline text** — Task and approval text with escaped multiline content is normalized on storage so paragraph breaks survive. ([#4444](https://github.com/paperclipai/paperclip/pull/4444))
...
- **Disappearing task comments** — Fixed comments occasionally disappearing from the task thread. ([#4557](https://github.com/paperclipai/paperclip/pull/4557))
- **External ticket URL rewriting in markdown** — Fixed external ticket URLs being incorrectly rewritten in rendered markdown. ([#4558](https://github.com/paperclipai/paperclip/pull/4558))
...
- **Stale execution run locks** — Fixed stale task execution run locks lingering after failed runs. ([#4258](https://github.com/paperclipai/paperclip/pull/4258))
...
- **API route authorization hardening** — Tightened actor, company, and active-checkout boundary enforcement across the REST surface for company, agent, plugin, adapter, cost, invite, and task mutation routes. Adds regression coverage for the audited routes. ([#4122](https://github.com/paperclipai/paperclip/pull/4122))
...
- Task reference mentions (`0060`)
- Heartbeat scheduled retry tracking (`0061`)
- Routine run dispatch fingerprint and origin-fingerprint uniqueness for routine-execution tasks (`0062`)
- Environments and environment leases (`0065`)
- Task tree holds for subtree pause/cancel/restore (`0066`)
