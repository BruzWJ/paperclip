...
export const taskChatLongThreadAgentMap = new Map<string, Agent>([
...
function markdownBody(index: number) {
  return [
    `## Baseline note ${index}`,
    "",
    `This assistant update captures a deterministic markdown-heavy row for long-thread rendering. It references [PAP-${2600 + index}](/PAP/tasks/PAP-${2600 + index}) and includes enough structure to exercise markdown parsing.`,
    "",
...
function plainUserBody(index: number) {
  return `Board checkpoint ${index}: keep the task-detail page responsive while the thread is full of historical comments.`;
}
...
function createComment(index: number): TaskChatComment {
...
export const taskChatLongThreadComments: TaskChatComment[] = Array.from(
  { length: LONG_THREAD_COMMENT_COUNT },
  (_, index) => createComment(index),
);

export const taskChatLongThreadMarkdownCommentIds = new Set(
  taskChatLongThreadComments
    .slice(0, LONG_THREAD_MARKDOWN_COMMENT_COUNT)
    .map((comment) => comment.id),
);

export const taskChatLongThreadEvents: TaskTimelineEvent[] = Array.from(
...
export const taskChatLongThreadFixtureContext = {
  task: {
    identifier: "PAP-PERF",
  ...
  subTasks: [
    ...
    "Phase 2: Isolate task-thread row rendering and Markdown work",
    "Phase 3: Apply virtualization and guard scroll behavior",
    "Phase 4: Verify production task profile improvement",
  ],
  sidebarStats: [
