...
export function validateRoutinesSearch(search: Record<string, unknown>): {
  ...
  if (
    folder !== undefined &&
    folder !== "unfiled" &&
    !isCanonicalUuid(folder)
  ) {
    throw new Error(
      'Invalid search parameter "folder": must be unfiled or a canonical UUID',
    );
  }
  return {
...
const RECENT_RUNS_TASK_DETAIL_LOCATION_STATE =
  createTaskDetailLocationState("routine_runs");
...
export function buildRoutineGroups(
  ...
  if (groupByValue === "project") {
    const groups = groupBy(
      routines,
      (routine) => routine.projectId ?? "__no_project",
    );
    return Object.keys(groups)
      .sort((left, right) => {
        const leftLabel =
          left === "__no_project"
            ? "No project"
            : (projectById.get(left)?.name ?? "Unknown project");
        const rightLabel =
          right === "__no_project"
            ? "No project"
            : (projectById.get(right)?.name ?? "Unknown project");
  ...
  const groups = groupBy(
    routines,
    (routine) => routine.assigneeAgentId ?? "__unassigned",
  );
  return Object.keys(groups)
    .sort((left, right) => {
      const leftLabel =
        left === "__unassigned"
          ? "Unassigned"
          : (agentById.get(left)?.name ?? "Unknown agent");
      const rightLabel =
        right === "__unassigned"
          ? "Unassigned"
          : (agentById.get(right)?.name ?? "Unknown agent");
      return leftLabel.localeCompare(rightLabel);
    })
    .map((key) => ({
      key,
...
function Routines() {
  ...
  const [statusMutationRoutineId, setStatusMutationRoutineId] = useState<
    string | null
  >(null);
  const [runDialogRoutine, setRunDialogRoutine] =
  ...
  const {
    ...
    queryFn: () =>
      tasksApi.list(companyId, { originKind: "routine_execution" }),
  ...
  const createRoutine = useMutation({
    mutationFn: () =>
      routinesApi.create(companyId, buildRoutineMutationPayload(draft)),
      ...
      pushToast({
        ...
        body:
          mutationError instanceof Error
            ...
            : "Paperclip could not save the folder.",
        tone: "error",
      });
    },
  });
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body:
          mutationError instanceof Error
            ? mutationError.message
            : "Paperclip could not update the folder.",
        tone: "error",
      });
    },
  });
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body:
          mutationError instanceof Error
            ? mutationError.message
            : "Paperclip could not delete the folder.",
        tone: "error",
      });
    },
  });
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body:
          mutationError instanceof Error
            ? mutationError.message
            : "Paperclip could not move the routine.",
        tone: "error",
      });
    },
  });
  ...
  const folderFilteredRoutines = useMemo(() => {
    ...
    if (folderSelection === "unfiled")
      return visibleRoutines.filter((routine) => !routine.folderId);
    return visibleRoutines.filter(
      (routine) => routine.folderId === folderSelection,
    );
  ...
  const currentProject = draft.projectId
    ...
    : null;
  const activeFolder = selectedFolderFromList(
  ...
  function handleTabChange(tab: string) {
    ...
    startTransition(() => {
      void routeNavigate({
        search: (previous) => ({
          ...previous,
          tab: nextTab === "runs" ? "runs" : undefined,
        }),
      });
    });
  }
  ...
  async function moveSelectedRoutines(folderId: string | null) {
    ...
    try {
      ...
      pushToast({
        title: "Routines moved",
        body: `${ids.length} routine${ids.length === 1 ? "" : "s"} filed.`,
        tone: "success",
      });
    } catch (moveError) {
      pushToast({
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    ...
    if (!routine.latestRevisionId) {
      pushToast({
        title: "Routine cannot be updated",
        body: "This routine has no canonical revision.",
        tone: "error",
      });
      return;
    }
                ...
                <PopoverTrigger asChild>
                  ...
                  >
                    <ArrowUpDown
                      ...
                      className="h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"
                    />
        ...
        >
          ...
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Define the recurring work first. Default project and agent are
                optional for draft routines.
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="text-sm text-muted-foreground">
              After creation, Paperclip takes you straight to trigger setup.
              Draft routines stay paused until you add a default agent.
            </div>
          ...
          {showFolderRail ? (
            <FolderRail
              ...
              onRename={(folder, name) =>
                updateFolder.mutate({ folderId: folder.id, payload: { name } })
              }
              onEdit={(folder) => {
            ...
            {visibleRoutines.length === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {routineSections.map((group) => {
                  const isOpen = !routineViewState.collapsedGroups.includes(
                    group.key,
                  );
                  return (
                    <Collapsible
                      ...
                      onOpenChange={(open) => {
                        updateRoutineView({
                          collapsedGroups: open
                            ? routineViewState.collapsedGroups.filter(
                                (item) => item !== group.key,
                              )
                          ...
                          <RoutineListRow
                            ...
                            onSelectChange={(selectedRoutine, selected) => {
                              setSelectedRoutineIds((current) =>
                                selected
                                  ? Array.from(
                                      new Set([...current, selectedRoutine.id]),
                                    )
                                  : current.filter(
                                      (id) => id !== selectedRoutine.id,
                                    ),
                              );
                            }}
                            extraMenuItems={
      ...
      <FolderFormDialog
        ...
        onSubmit={(payload) => {
          if (folderDialogTarget)
            updateFolder.mutate({ folderId: folderDialogTarget.id, payload });
