...
function RoutineOverviewRoute() {
  ...
  return <RoutineDetail companyId={companyId} routineId={routineId} />;
}
...
export function RoutineDetail({
  ...
  const dirtyFields = useMemo<DirtyFieldDescriptor[]>(() => {
    ...
    if (
      ...
      JSON.stringify(routineDefaults.env ?? null)
    ) {
  ...
  }, [routine, routineDefaults, isEditDirty, setBreadcrumbs, companyId]);
  ...
  const onHistoryRestoreSecretMaterials = useCallback(
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-background px-6">
          ...
          <div className="ml-auto flex shrink-0 items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span
                className={`text-sm font-medium ${automationLabelClassName}`}
              >
                {automationLabel}
              </span>
            </div>
