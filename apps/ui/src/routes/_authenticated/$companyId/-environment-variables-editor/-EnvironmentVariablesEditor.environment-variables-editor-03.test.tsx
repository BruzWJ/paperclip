...
import { SecretPicker } from "../-EnvironmentVariableSecretPicker";
...
describe("EnvironmentVariablesEditor", () => {
  ...
  it("lets the user dismiss the sensitive-value hint, unmasking the value and keeping it plain (§6.6)", async () => {
    ...
    expect(container.querySelector('button[aria-label="Dismiss sensitive-value suggestion"]')).toBeNull();
    expect(container.querySelector<HTMLInputElement>('input[aria-label="Variable value"]')!.type).toBe(
      "text",
    );
  ...
  it("lets slash-delimited secrets be browsed by folder and exposes full paths on hover", async () => {
    ...
    render(
      <SecretPicker
        ...
        secrets={[
          ...
          makeSecret("plain-key", {
            name: "paperclip-page-aws-access-key-id",
            key: "paperclip-page-aws-access-key-id",
          }),
        ]}
        onSelect={onSelect}
        onCreateNew={() => {}}
    ...
    const itemByText = (text: string) =>
      [...document.querySelectorAll<HTMLElement>("[cmdk-item]")].find((el) => el.textContent?.includes(text));
    const rowTitle = (item: HTMLElement | undefined) =>
      item?.querySelector<HTMLElement>("[title]")?.getAttribute("title");

    expect(rowTitle(itemByText("paperclip-cloud"))).toBe("/paperclip-cloud");
    expect(rowTitle(itemByText("paperclip-page-aws-access-key-id"))).toBe("paperclip-page-aws-access-key-id");
