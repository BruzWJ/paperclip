...
export function parseCompanyExportFilePath(
...
function CompanyExportRoute() {
  ...
  return <CompanyExport filePath={filePath} />;
}
  ...
  return nodes
    .map((node) => {
      if (node.kind === "dir") {
        return {
          ...node,
          children: sortByChecked(node.children, checkedFiles),
        };
      }
      return node;
    })
...
function downloadZip(
  ...
  for (const [path] of Object.entries(exported.files)) {
    if (selectedFiles.has(path))
      filteredFiles[path] = effectiveFiles[path] ?? exported.files[path];
  }
...
export function CompanyExport({
  ...
  // Sync selectedFile from the typed splat prop on browser back/forward.
  useEffect(() => {
