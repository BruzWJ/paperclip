...
function providerConfig(
  overrides: Partial<CompanySecretProviderConfig> &
    Pick<CompanySecretProviderConfig, "id" | "provider">,
): CompanySecretProviderConfig {
  return {
    companyId: "company-1",
...
describe("Secrets page provider helpers", () => {
  ...
  it("blocks provider modes the backend does not support", () => {
    expect(getCreateProviderBlockReason(localProvider, "external", null)).toBe(
      "Local encrypted (default) does not support linked external references.",
    );
  });
