...
function makeUserSecretDefinition(
...
async function selectRadixOption(trigger: HTMLElement, optionText: string) {
  ...
  const option = Array.from(document.querySelectorAll('[role="option"]')).find(
    (el) => el.textContent?.trim().startsWith(optionText),
  );
...
describe("Secrets page layout", () => {
  ...
  beforeEach(() => {
    ...
    mockSecretsApi.providerConfigDiscoveryPreview.mockResolvedValue(
      makeDiscoveryPreview(),
    );
    mockSecretsApi.remoteImportPreview.mockResolvedValue(
      makeRemoteImportPreview(),
    );
    mockSecretsApi.listUserSecretDefinitions.mockResolvedValue([]);
    mockSecretsApi.userSecretDefinitionCoverage.mockResolvedValue(
      userSecretCoverage,
    );
  ...
  it("uses the shared search/filter/tab affordances and keeps vault sections quiet", async () => {
    ...
    expect(
      container.querySelector(
        'input[data-page-search-target="true"][aria-label="Search secrets"]',
      ),
    ).not.toBeNull();
    ...
    expect(
      container.querySelector('a[href="#provider-vaults-local_encrypted"]'),
    ).not.toBeNull();
  ...
  it("refreshes existing AWS secrets from a provider vault card", async () => {
    ...
    expect(mockSecretsApi.remoteImportPreview).toHaveBeenCalledWith(
  ...
  it("warns that removing a provider vault only removes Paperclip config", async () => {
    mockSecretsApi.removeProviderConfig.mockResolvedValueOnce(
      providerConfigs[1],
    );
    const root = createRoot(container);
    const queryClient = new QueryClient({
    ...
    expect(mockSecretsApi.removeProviderConfig).toHaveBeenCalledWith(
      "vault-aws",
    );
  ...
  it("keeps references reachable from the compact secrets row and detail drawer", async () => {
    ...
    const viewUsageButton = Array.from(
      document.body.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("View in Usage")) as
  ...
  it("merges company secrets and each-user definitions into the Secrets list", async () => {
    ...
    await waitForReact(
      () => container.textContent?.includes("3/5 set") ?? false,
    );
    ...
    const cardView = container.querySelector(
    ...
    // Grid template lives in the token layer: --gtc-54 in apps/ui/src/index.css
    // carries the original minmax(12rem,2.4fr)... template verbatim.
    expect(tableView?.querySelector("[role='row']")?.className).toContain(
      "grid-cols-(--gtc-54)",
    );
    expect(cardView?.className).toContain("@min-[40rem]:hidden");
    expect(cardView?.className).not.toContain("md:hidden");

    expect(mockSecretsApi.list).toHaveBeenCalledWith(
      "11111111-1111-4111-8111-111111111111",
    );
  ...
  it("creates an each-user secret from the unified New secret dialog", async () => {
    ...
    const newSecretButton = Array.from(
      container.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("New secret")) as
      HTMLButtonElement | undefined;
    await act(async () => {
    ...
    const companyKeyInput = document.getElementById(
      "new-secret-key",
    ) as HTMLInputElement;
    expect(companyKeyInput.readOnly).toBe(true);

    const editKeyButton = Array.from(
      document.body.querySelectorAll("button"),
    ...
    const eachUserButton = Array.from(
      document.body.querySelectorAll("button"),
    ).find((button) => button.textContent?.trim() === "Each user") as
      HTMLButtonElement | undefined;
    await act(async () => {
      eachUserButton?.dispatchEvent(
        new PointerEvent("pointerdown", { bubbles: true }),
      );
      eachUserButton?.dispatchEvent(
        new KeyboardEvent("keydown", { bubbles: true, key: "Enter" }),
    ...
    const usageGuidance = document.getElementById(
      "new-secret-usage-guidance",
    ...
    await act(async () => {
      ...
      setTextareaValue(
        usageGuidance,
        "Create a fine-grained PAT with repo read access.",
      );
    });
    await flushReact();
  ...
  it("opens the New secret dialog when provider queries fail", async () => {
    mockSecretsApi.providers.mockRejectedValueOnce(
      new ApiError("Providers unavailable", 403, null),
    );
  ...
  it("opens the each-user detail sheet with coverage and set-my-value actions", async () => {
    ...
    const viewCoverageButton = Array.from(
      document.body.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("View in Coverage")) as
      HTMLButtonElement | undefined;
    await act(async () => {
    ...
    const setValueButton = Array.from(
      document.body.querySelectorAll("button"),
    ).find((button) => button.textContent?.includes("Set my value")) as
      HTMLButtonElement | undefined;
    await act(async () => {
  ...
  it("explains AWS managed secret creation failures with actionable safe details", async () => {
    ...
    await act(async () => {
      setInputValue(
        document.getElementById("new-secret-name") as HTMLInputElement,
        "AWS test token",
      );
      setTextareaValue(
        document.getElementById("new-secret-value") as HTMLTextAreaElement,
        "secret-value",
      );
    });
    ...
    const createButton = Array.from(
      document.body.querySelectorAll("button"),
    ).find((button) => button.textContent?.trim() === "Create secret") as
      HTMLButtonElement | undefined;
    await act(async () => {
      createButton?.click();
    });
    ...
    expect(errorBanner?.textContent).toContain(
      "link it as an external reference",
    );
  ...
  it("renders generic secret creation failures with a stable selector", async () => {
    mockSecretsApi.create.mockRejectedValueOnce(
      new ApiError("Secret creation failed", 500, null),
    );
    const root = createRoot(container);
    const queryClient = new QueryClient({
      defaultOptions: { queries: { retry: false } },
  ...
  it("discovers AWS provider vault candidates and applies selected values as prefill", async () => {
    ...
    expect(
      (document.getElementById("provider-vault-namespace") as HTMLInputElement)
        .value,
    ).toBe("prod-use1");
    expect(
      (
    ...
    expect(
      (document.getElementById("provider-vault-kms-key-id") as HTMLInputElement)
        .value,
    ).toBe("alias/paperclip-secrets");
    expect(
      (document.getElementById("provider-vault-owner-tag") as HTMLInputElement)
        .value,
    ).toBe("platform");
    expect(
      (
        document.getElementById(
          "provider-vault-environment-tag",
        ) as HTMLInputElement
      ).value,
    ).toBe("production");
  ...
  it("shows AWS discovery errors without replacing manual vault form values", async () => {
    ...
    const namespaceInput = document.getElementById(
  ...
  it("keeps generic AWS discovery 403 errors on the generic failure path", async () => {
    ...
    expect(errorBanner?.textContent).toContain(
      "AWS discovery request failed before IAM evaluation.",
    );
  ...
  it("auto-generates the key from the name and keeps it read-only until Edit", async () => {
    ...
    expect(
      nameInput.compareDocumentPosition(valueTextarea) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(
      valueTextarea.compareDocumentPosition(keyInput) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    ...
    expect(
      (document.getElementById("new-secret-key") as HTMLInputElement).readOnly,
    ).toBe(false);

    await act(async () => {
      setInputValue(
        document.getElementById("new-secret-key") as HTMLInputElement,
        "custom-key",
      );
    ...
    expect(
      (document.getElementById("new-secret-key") as HTMLInputElement).value,
    ).toBe("custom-key");
  ...
  it("shows an empty AWS discovery result without blocking manual entry", async () => {
    ...
    expect(document.body.textContent).toContain(
      "No AWS vault metadata candidates found",
    );
...
describe("Secrets folder view (PAP-14698)", () => {
  ...
  it("renders the empty-folder state (breadcrumb intact) for an unknown path", async () => {
    ...
    expect(
      (document.getElementById("new-secret-name") as HTMLInputElement).value,
    ).toBe("");
    expect(
      document.querySelector('button[aria-label="Remove folder prefix"]'),
    ).not.toBeNull();
  ...
  it("distinguishes a filtered-empty folder from a genuinely empty folder", async () => {
    ...
    await waitForReact(
      () =>
        container.textContent?.includes("No secrets match your filters.") ??
        false,
    );
  ...
  it("creates a company secret from a folder prefix and derives the key from the full name", async () => {
    ...
    expect(
      (document.getElementById("new-secret-key") as HTMLInputElement).value,
    ).toBe("dev-github-oauth-clientsecret-deeper");
    await act(async () =>
      setTextareaValue(
        document.getElementById("new-secret-value") as HTMLTextAreaElement,
        "secret-value",
  ...
  it("keeps the folder prefix for Each user and exposes the full name when the chip is removed", async () => {
    ...
    expect(
      (document.getElementById("new-secret-key") as HTMLInputElement).value,
    ).toBe("DEV_GITHUB_OAUTH_PERSONAL_TOKEN");
    ...
    expect(
      (document.getElementById("new-secret-name") as HTMLInputElement).value,
    ).toBe("dev/github/oauth/personal-token");
    expect(
      document.querySelector('button[aria-label="Remove folder prefix"]'),
    ).toBeNull();
  ...
  it("validates New folder inline and stages the trimmed segment in the URL-backed folder view", async () => {
    ...
    expect(container.textContent).toContain("No secrets in this folder yet.");
    expect(
      container.querySelector('input[aria-label="Folder name"]'),
    ).toBeNull();
