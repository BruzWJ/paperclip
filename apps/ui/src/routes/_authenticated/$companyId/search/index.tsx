...
export function validateCompanySearch(
  ...
  const validated = {
    ...
    priority: optionalSearchEnumArray(
      search.priority,
      TASK_PRIORITIES,
      "priority",
    ),
    ownerAgentId: optionalCanonicalUuidSearch(
      search.ownerAgentId,
      "ownerAgentId",
    ),
    ...
    updatedAfter: optionalExactIsoTimestampSearch(
      search.updatedAfter,
      "updatedAfter",
    ),
  };
  if (
    validated.ownerAgentId !== undefined &&
    validated.ownerUserId !== undefined
  ) {
    throw new Error(
...
type SubGroupKey =
  "tasks" | "comments" | "documents" | "artifacts" | "agents" | "projects";
...
function isCompanySearchScope(value: unknown): value is CompanySearchScope {
  return (
    Boolean(value) &&
...
function searchFiltersFromRoute(
  ...
  if (search.ownerAgentId !== undefined)
    filters.ownerAgentId = search.ownerAgentId;
  if (search.ownerUserId !== undefined)
    filters.ownerUserId = search.ownerUserId;
  ...
  if (search.updatedWithin !== undefined)
    filters.updatedWithin = search.updatedWithin;
  if (search.updatedAfter !== undefined)
    filters.updatedAfter = search.updatedAfter;
  return filters;
}
...
function Search() {
  ...
  const labelsById = useMemo(
    () => new Map((labels as TaskLabel[]).map((l) => [l.id, l])),
    [labels],
  );
      ...
      return {
        ...
        label: (
          <span className="flex items-center">
            ...
            {dashOut ? (
              <span className="ml-1.5 text-(length:--text-nano) text-muted-foreground">
                —
              </span>
            ) : count !== null ? (
              pill(count)
      ...
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Tasks, comments, plan documents, artifacts, agents, projects — same
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            ...
            to jump straight to a task.
          </li>
          <li>
            <span className="font-medium text-foreground">Quoted phrases:</span>{" "}
            wrap a phrase in quotes to match the exact sequence.
          </li>
          <li>
            <span className="font-medium text-foreground">⌘K:</span> reopens the
            command palette pre-seeded with your current query.
          </li>
        </ul>
      </div>
    );
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          {status ? `The server returned ${status}.` : "The request failed."}{" "}
          Your input and filters are still here, so you can retry or fall back
          to the Tasks filter.
        </p>
  ...
  if (isEmpty) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">
          We couldn’t find a match in {describeScope(scope).toLowerCase()}. Try
          widening the scope or rephrasing your query.
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          ...
          <li>
            Search by a task title or a distinctive phrase from its content.
          </li>
