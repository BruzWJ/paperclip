...
function createZipArchiveWithDirectoryEntries(
  rootPath: string,
  additionalEntries: Array<{ path: string; body: string }> = [],
) {
  const encoder = new TextEncoder();
  const entries = [
    ...
    {
      path: `${rootPath}/agents/lead/AGENTS.md`,
      body: encoder.encode("# Lead\n"),
      compressionMethod: 8,
    },
    ...additionalEntries.map((entry) => ({
      path: entry.path,
      body: encoder.encode(entry.body),
      compressionMethod: 0,
    })),
  ].map((entry) => ({
    ...entry,
...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe(
      "paperclip-demo/agents/lead/AGENTS.md",
    );
  ...
  it("rejects noncanonical archive paths instead of normalizing them", async () => {
    const archive = createDeflatedZipArchive(
      { "agents\\lead\\AGENTS.md": "# Lead\n" },
      "paperclip-demo",
    );

    await expect(readZipArchive(archive)).rejects.toThrow(
      "entry paths must be exact portable relative paths",
    );
    expect(() =>
      createZipArchive(
        { "agents\\lead\\AGENTS.md": "# Lead\n" },
    ...
    ).toThrow("entry paths must be exact portable relative paths");
  });

  it("rejects duplicate archive entries instead of overwriting them", async () => {
    const archive = createZipArchiveWithDirectoryEntries("paperclip-demo", [
      {
        path: "paperclip-demo/COMPANY.md",
        body: "# Replacement\n",
      },
    ]);
