...
const QA_ID = "abcdefab-cdef-4abc-8def-abcdefabcdef";
...
describe("owner transition helpers", () => {
  it("distinguishes operator interruption from an ordinary cancellation", () => {
    ...
    expect(
      resolveRunStatusPresentation("cancelled", { operatorInterrupted: true })
        .label,
    ).toBe("interrupted");
  ...
  it("recognizes only structured agent mentions as notifications", () => {
    ...
    expect(
      findPlainAgentNameCandidate("please ask QA", [
  ...
  it("uses owner language for the live-run confirmation", () => {
    expect(
      describeOwnerChangeInterrupt({ runningAgentName: "ClaudeCoder" }),
    ).toMatchObject({
      banner:
        "ClaudeCoder is running — changing the owner will interrupt this run.",
