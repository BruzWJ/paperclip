...
5. Propose a registered \`adapterType\` that the existing Paperclip worker can execute.
...
8. Never place a generic Paperclip credential, bridge, or Paperclip-managed session selector in provider configuration. The existing Paperclip worker executes the approved adapter configuration and Paperclip delivers a run-scoped compiled tool interface for each invocation.
