...
describe("deriveMonitorState", () => {
  ...
  it("derives due-now and overdue at the grace boundary", () => {
    const task = (nextCheckAt: string) => ({
      executionState: { monitor: { status: "scheduled" as const, nextCheckAt, attemptCount: 1 } },
    });

    expect(deriveMonitorState(task("2026-07-17T19:59:00.001Z"), now).state).toBe("due-now");
    expect(deriveMonitorState(task("2026-07-17T19:59:00.000Z"), now).state).toBe("overdue");
  });
