...
function taskTreeControlLabel(mode: TaskTreeControlMode, scope: "leaf" | "subtree") {
  return scope === "leaf"
    ? LEAF_WORK_CONTROL_MODE_LABEL[mode] ?? TREE_CONTROL_MODE_LABEL[mode]
    : TREE_CONTROL_MODE_LABEL[mode];
}

function taskTreeControlHelpText(mode: TaskTreeControlMode, scope: "leaf" | "subtree") {
  return scope === "leaf"
...
function readTaskRunStateFromCache(
...
function TaskAttributionByline({
  ...
  const owner: AttributionActor | null = task.ownerAgentId
    ...
    : task.ownerUserId
      ? {
          kind: "user",
          id: task.ownerUserId,
          name: formatUserLabel(task.ownerUserId, userLabelMap)
  ...
  return (
    <AvatarGroup className="-space-x-1.5" aria-label="Task people" data-testid="task-attribution-avatar-stack">
...
const TaskDetailChatTab = memo(function TaskDetailChatTab({
  ...
  const { data: activeRunPage } = useQuery({
    ...
    queryFn: () => runsApi.listForTask(taskId, {
...
function TaskDetailActivityTab({
  ...
  const { data: taskTreeCostSummary } = useQuery({
    ...
    placeholderData: keepPreviousDataForSameQueryTail<Awaited<ReturnType<typeof tasksApi.getCostSummary>>>(taskId),
  });
      ...
      {hasTaskTreeCost && taskTreeCostSummary && (
        <div className="mb-3 px-3 py-2 rounded-lg border border-border">
          ...
          <div className="flex flex-wrap gap-3 text-xs text-muted-foreground tabular-nums">
            ...
            <span>{taskTreeCostSummary.taskCount} task{taskTreeCostSummary.taskCount === 1 ? "" : "s"}</span>
          </div>
        </div>
      )}
      <div className="mb-3">
...
export function TaskDetail() {
  ...
  const {
    ...
    placeholderData: keepPreviousDataForSameQueryTail<InfiniteData<BoardTaskCommentGroupPage, string | null>>(taskId ?? "pending"),
  });
  const comments = useMemo(
    () => flattenBoardTaskCommentGroupPages(commentPages?.pages, {
      companyId: resolvedCompanyId ?? "",
      taskId: taskId!,
    }, commentGroupContinuations),
    [commentGroupContinuations, commentPages?.pages, taskId, resolvedCompanyId],
  );
  const loadMoreCommentGroup = useCallback(async (rootCommentId: string) => {
    if (!taskId || loadingCommentGroupRootsRef.current.has(rootCommentId)) return;
  ...
  const { data: workProducts } = useQuery({
    ...
    enabled: !!taskId,
    placeholderData: keepPreviousDataForSameQueryTail<TaskWorkProduct[]>(taskId ?? "pending"),
  });
  ...
  const { data: activeTaskRunPage } = useQuery({
    ...
    refetchInterval: 3000,
    placeholderData:
      keepPreviousDataForSameQueryTail<TaskExecutionRunListPageRecord>(taskId ?? "pending"),
  });
  ...
  const {
    ...
    queryKey:
      task?.parentId && resolvedCompanyId
        ? queryKeys.tasks.listByParent(resolvedCompanyId, task.parentId)
        : ["tasks", "siblings", "pending"],
    queryFn: () => tasksApi.list(resolvedCompanyId!, { parentId: task!.parentId!, includeBlockedBy: true }),
    enabled: !!resolvedCompanyId && !!task?.parentId,
  });
  ...
  const sharedCompanyRuns = useSharedPollingQuery<TaskExecutionRunListPageRecord>({
    companyId: resolvedCompanyId,
  ...
  const { data: companyRunPage, dataUpdatedAt: companyRunsUpdatedAt } = useQuery({
    ...
    placeholderData:
      keepPreviousDataForSameQueryTail<TaskExecutionRunListPageRecord>(resolvedCompanyId ?? "pending"),
  });
  ...
  const { data: taskOwnerCatalog } = useQuery({
    queryKey: queryKeys.agents.taskOwnerCatalog(selectedCompanyId!),
    queryFn: () => agentsApi.listInvokableTaskOwners(selectedCompanyId!),
    enabled: !!selectedCompanyId,
  });
  ...
  const { data: mentionTasks = [] } = useQuery({
    queryKey: resolvedCompanyId ? queryKeys.tasks.mentionPool(resolvedCompanyId) : ["tasks", "mention-pool", "pending"],
    queryFn: () => tasksApi.list(resolvedCompanyId!, { limit: 100, sortField: "updated", sortDir: "desc" }),
    enabled: !!resolvedCompanyId,
    staleTime: 60_000,
    placeholderData: keepPreviousDataForSameQueryTail<Task[]>(resolvedCompanyId ?? "pending"),
  });
  ...
  const commentOwnerOptions = useMemo(() => {
    ...
    const activeAgents = [...(taskOwnerCatalog ?? [])]
  ...
  const taskCacheRefs = useMemo(() => {
    const refs = new Set<string>();
    if (taskId) refs.add(taskId);
    if (task?.id) refs.add(task.id);
    if (task?.identifier) refs.add(task.identifier);
    return [...refs];
  }, [task?.id, task?.identifier, taskId]);

  const invalidateTaskDetail = useCallback(() => {
    for (const ref of taskCacheRefs) {
      queryClient.invalidateQueries({ queryKey: queryKeys.tasks.detail(ref) });
      queryClient.invalidateQueries({ queryKey: queryKeys.tasks.activity(ref) });
    }
  }, [taskCacheRefs, queryClient]);
  const invalidateTaskThreadLazily = useCallback(() => {
    for (const ref of taskCacheRefs) {
      queryClient.invalidateQueries({ queryKey: queryKeys.tasks.detail(ref), refetchType: "inactive" });
      queryClient.invalidateQueries({ queryKey: queryKeys.tasks.activity(ref), refetchType: "inactive" });
    }
  }, [taskCacheRefs, queryClient]);

  const invalidateTaskRunState = useCallback(() => {
    for (const ref of taskCacheRefs) {
      queryClient.invalidateQueries({ queryKey: ["tasks", "runs", ref] });
    }
  }, [taskCacheRefs, queryClient]);

  const invalidateTaskDocumentAnnotationState = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ["tasks", "document-annotations", taskId!] });
  ...
  const upsertCommentInCache = useCallback((_comment: unknown) => {
    for (const ref of taskCacheRefs) {
      queryClient.invalidateQueries({ queryKey: queryKeys.tasks.comments(ref) });
    }
  }, [taskCacheRefs, queryClient]);
  ...
  const updateTaskTitle = useMutation({
    ...
    onMutate: async (title) => {
      ...
      applyOptimisticTaskCacheUpdate(taskRefs, { title });
  ...
  const reassignTask = useMutation({
    ...
    onSuccess: ({ task: nextTask }) => {
      const refs = new Set<string>([taskId!, nextTask.id]);
      if (nextTask.identifier) refs.add(nextTask.identifier);
      mergeTaskResponseIntoCaches(refs, nextTask);
    ...
    mutationFn: async () => {
      ...
      if (task.ownerKind === "agent" && task.ownerAgentId) {
        ...
        mergeTaskResponseIntoCaches(
          new Set([task.id, task.identifier].filter(Boolean) as string[]),
          assigned.task,
        );
      }
      if (
        ...
        withdrawalTask.ownerAssignmentSource !==
        ...
        await Promise.all([
          ...
          ...(task?.id
            ? [
                ...
                queryClient.invalidateQueries({ queryKey: queryKeys.tasks.listByDescendantRoot(selectedCompanyId, task.id) }),
              ]
            : []),
        ]);
      }
  ...
  const archiveFromInbox = useMutation({
    ...
    onMutate: async (id) => {
      if (!selectedCompanyId) return { previousData: [] as InboxTaskCacheSnapshot };
      beginLocalInboxArchive(selectedCompanyId, id);
      await cancelInboxTaskQueries(queryClient, selectedCompanyId);
      const previousData = snapshotInboxTaskCaches(queryClient, selectedCompanyId);
  ...
  useEffect(() => {
    ...
    lastScrollTaskIdRef.current = taskId;
    if (!shouldScrollTaskDetailToTopOnNavigation({ previousTaskId, nextTaskId: taskId, navigationType })) return;
  ...
  const canPauseLeafWork = canManageTreeControl && childTasks.length === 0 && !activePauseHold && !isTerminalTask;
  const canResumeLeafWork = canManageTreeControl && childTasks.length === 0 && activePauseHold?.isRoot === true;
          ...
          {(task.labels ?? []).length > 0 && (
            <div className="hidden sm:flex items-center gap-1">
              ...
              {(task.labels ?? []).length > 4 && (
                <span className="text-(length:--text-nano) text-muted-foreground">+{(task.labels ?? []).length - 4}</span>
              )}
            </div>
          )}
