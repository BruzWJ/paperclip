...
describe("typing-dots CSS animation guard (PAP-54 failure mode)", () => {
  ...
  // Locate apps/ui/src/index.css regardless of whether vitest runs from apps/ui/ or
  ...
  function readIndexCss(): string {
    ...
    for (let depth = 0; depth < 6; depth++) {
      for (const candidate of [
        ...
        path.join(dir, "apps/ui/src/index.css"),
      ]) {
        if (existsSync(candidate)) return readFileSync(candidate, "utf8");
      }
      dir = path.dirname(dir);
    }
    throw new Error("apps/ui/src/index.css not found from " + process.cwd());
  }
