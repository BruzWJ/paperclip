...
export function TaskChatLongThreadPerf() {
  ...
  const fixture = taskChatLongThreadFixtureContext;
  const rowTarget = useMemo(
    () => LONG_THREAD_COMMENT_COUNT + taskChatLongThreadEvents.length,
    [],
  );
  ...
  return (
    <div data-testid="task-chat-long-thread-perf" className="space-y-5">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="outline" className="font-mono text-(length:--text-micro)">
              {fixture.task.identifier}
            </Badge>
            <Badge variant="secondary">
              {fixture.task.boardPresentationStatus.replace(/_/g, " ")}
            </Badge>
            <Badge variant="outline">{fixture.task.projectName}</Badge>
          </div>
          <h1 className="mt-3 text-2xl font-semibold tracking-tight">{fixture.task.title}</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
            Deterministic local fixture for measuring the current direct-render task chat path with
        ...
        <main className="min-w-0 space-y-4">
          <Card className="border-border/70">
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Task documents</CardTitle>
            </CardHeader>
          ...
          <Card className="border-border/70">
            ...
            <CardContent className="space-y-2">
              {fixture.subTasks.map((subTask, index) => (
                <div key={subTask} className="flex items-center gap-3 rounded-md border border-border bg-background px-3 py-2 text-sm">
          ...
          <Profiler id="task-chat-long-thread" onRender={handleRender}>
            <TaskChatThread
          ...
          <Card className="border-border/70">
            ...
            <CardContent className="space-y-2">
              ...
              <div className="hidden" data-testid="perf-markdown-comment-id-sample">
                {[...taskChatLongThreadMarkdownCommentIds].slice(0, 3).join(",")}
              </div>
            </CardContent>
          </Card>
        </aside>
