...
import "./RoutineHistoryTab.test-support";
...
import {
  ...
  snapshotV1,
  toastSpy,
  useMockRoutinesApiTestState,
} from "./RoutineHistoryTab.test-support";
const mockRoutinesApi = useMockRoutinesApiTestState();
describe("RoutineHistoryTab", () => {
  ...
  it("calls restoreRevision and surfaces a success toast after confirming restore", async () => {
    ...
    await flush();
    expect(mockRoutinesApi.restoreRevision).toHaveBeenCalledWith("routine-1", "revision-1", {
      changeSummary: null,
    });
    expect(toastSpy).toHaveBeenCalled();
    const successCall = toastSpy.mock.calls.find((call) => call[0] === "Restored revision 1 as revision 3");
  ...
  it("shows env summary on the revision preview and routes counts into restore dialog", async () => {
    ...
    const old = createRevision({
      ...
      snapshot: snapshotV1({
        env: {
          GH_TOKEN: { type: "secret_ref", secretId: "secret-1", version: 3 },
        },
      }),
    });
    mockRoutinesApi.listRevisions.mockResolvedValue([current, old]);
