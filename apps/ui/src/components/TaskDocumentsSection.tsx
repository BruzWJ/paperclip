...
function loadFoldedDocumentKeys(taskId: string) {
  ...
  try {
    const raw = window.localStorage.getItem(
      getFoldedDocumentsStorageKey(taskId),
    );
    if (!raw) return [];
...
function makeTaskDocumentSubject(task: Task): DocumentSubjectConfig {
  return {
    id: task.id,
    ...
    documentRevisionsQueryKey: (key) =>
      queryKeys.tasks.documentRevisions(task.id, key),
    ...
    setDocumentLock: (key, locked) =>
      locked
        ? tasksApi.lockDocument(task.id, key)
        : tasksApi.unlockDocument(task.id, key),
...
export function TaskDocumentsSection({
  ...
  const documentSubject = useMemo(() => {
    ...
    if (!task)
      throw new Error("TaskDocumentsSection requires either task or subject");
  ...
  const syncDocumentCaches = useCallback(
    (document: TaskDocument) => {
      if (
        ...
        isSystemTaskDocumentKey(document.key)
      )
        return;
