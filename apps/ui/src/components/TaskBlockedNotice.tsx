...
function WaitingOnLiveWorkNotice({
  ...
  renderParkedChip: (blocker: TaskRelationTaskSummary) => ReactNode;
}) {
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          <div className="space-y-1" data-testid="task-blocked-notice-progress">
          ...
          <div data-testid="task-blocked-notice-steps">
            {steps.map(({ blocker, status }) => (
          ...
          {nowRunning.length > 0 ? (
            <div
              data-testid="task-blocked-notice-now-running"
...
export function TaskBlockedNotice({
  ...
  /** Company-wide set of task ids with a queued/running run (own or blocker). */
  ...
  // Rule C (PAP-13554 / plan §Rule C): when the task is `blocked` and a
  ...
  // (status ≠ done/cancelled), so a non-empty list on a `blocked` task is
