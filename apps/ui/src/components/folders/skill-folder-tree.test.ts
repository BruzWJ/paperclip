...
function folder(
  partial: Partial<FolderListItem> & { id: string; slug: string; path: string },
): FolderListItem {
  return {
    companyId: "co",
    kind: "skill",
...
describe("buildSkillFolderTree", () => {
  it("groups reserved roots and company folders in order", () => {
    ...
    expect(model.roots.map((n) => n.folder.id)).toEqual([
      "my",
      "eng",
      "proj",
      "bundled",
  ...
  it("nests children under their parent", () => {
    ...
    expect(model.company[0]?.children.map((n) => n.folder.id)).toEqual([
      "eng-review",
    ]);
    expect(model.childrenById.get("bundled")?.map((n) => n.folder.id)).toEqual([
      "bundled-git",
    ]);
  });
});
...
describe("subtreeFolderIds", () => {
  it("includes the folder and all descendants", () => {
    const model = buildSkillFolderTree(folders);
    expect([...subtreeFolderIds(model, "eng")].sort()).toEqual([
      "eng",
      "eng-review",
    ]);
...
describe("folderBreadcrumbTrail", () => {
  it("walks from top-level root down to the target", () => {
    const model = buildSkillFolderTree(folders);
    expect(folderBreadcrumbTrail(model, "eng-review").map((f) => f.id)).toEqual(
      ["eng", "eng-review"],
    );
    expect(folderBreadcrumbTrail(model, "mine").map((f) => f.id)).toEqual([
      "my",
      "mine",
    ]);
  });
...
describe("reserved subtree detection", () => {
  it("flags bundled root and descendants", () => {
    expect(isBundledFolder({ path: "bundled", systemKey: "bundled" })).toBe(
      true,
    );
    expect(
      isBundledFolder({ path: "bundled/git", systemKey: "bundled:git" }),
    ).toBe(true);
    expect(isBundledFolder({ path: "engineering", systemKey: null })).toBe(
      false,
    );
  });

  it("flags projects root and descendants", () => {
    expect(isProjectsFolder({ path: "projects", systemKey: "projects" })).toBe(
      true,
    );
    expect(
      isProjectsFolder({ path: "projects/acme", systemKey: "project:a" }),
    ).toBe(true);
    expect(isProjectsFolder({ path: "my/dotta", systemKey: "my:u1" })).toBe(
      false,
    );
  });
});
...
describe("reservedRootLabel", () => {
  it("renames reserved roots and passes through others", () => {
    expect(reservedRootLabel({ systemKey: "my", name: "my" })).toBe(
      "My Skills",
    );
    expect(reservedRootLabel({ systemKey: "projects", name: "projects" })).toBe(
      "Projects",
    );
    expect(reservedRootLabel({ systemKey: "bundled", name: "bundled" })).toBe(
      "Bundled",
    );
    expect(reservedRootLabel({ systemKey: null, name: "Engineering" })).toBe(
      "Engineering",
    );
  });
});
...
describe("skillFolderDisplayPath", () => {
  it("prefixes company folders and preserves reserved-root labels", () => {
    const model = buildSkillFolderTree(folders);
    expect(skillFolderDisplayPath(model, "eng-review")).toBe(
      "Company / Engineering / Review",
    );
...
describe("skillFolderPathDisplayFallback", () => {
  it("humanizes cold detail paths before folder metadata loads", () => {
    expect(skillFolderPathDisplayFallback("engineering/code-review")).toBe(
      "Company / Engineering / Code Review",
    );
    expect(skillFolderPathDisplayFallback("my/review-notes/drafts")).toBe(
      "My Skills / Review Notes / Drafts",
    );
    expect(skillFolderPathDisplayFallback("bundled/review-pr")).toBe(
      "Bundled / Review Pr",
    );
