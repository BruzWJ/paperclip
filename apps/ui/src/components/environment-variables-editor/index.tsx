...
function normalizedEnvEntries(
  ...
  for (const [rawName, binding] of Object.entries(value)) {
    ...
    if (binding.type === "secret_ref") {
      const secretId =
        typeof binding.secretId === "string" ? binding.secretId : "";
  ...
  return [...byName.entries()].sort(([left], [right]) =>
    left.localeCompare(right),
  );
}

function normalizedEnvKey(
  value: Record<string, EnvBinding> | null | undefined,
): string {
...
>(function EnvironmentVariablesEditor(
  ...
  const [committedRows, setCommittedRows] = useState<EnvRow[]>(() =>
    cloneRows(rows),
  );
  ...
  function markCommitted(
    nextValueKey: string,
    nextRows: readonly EnvRow[] = rowsRef.current,
  ) {
  ...
  useEffect(() => {
    ...
    const matchesPendingSave =
      pendingSaveValueKeyRef.current === incomingValueKey;
  ...
  const draftValueKey = useMemo(
    () => normalizedEnvKey(draftValue),
    [draftValue],
  );
  ...
  const changeSummary = useMemo(() => {
    const committed = new Map(
      normalizedEnvEntries(valueFromRows(committedRows)).map(
        ([name, binding]) => [name, JSON.stringify(binding)],
      ),
    );
    const draft = new Map(
      normalizedEnvEntries(draftValue).map(([name, binding]) => [
        name,
  ...
  const changeSummaryText = useMemo(() => {
    ...
    if (changeSummary.added.length > 0)
      parts.push(`New: ${formatChangedNames(changeSummary.added)}`);
    if (changeSummary.changed.length > 0)
      parts.push(`Edited: ${formatChangedNames(changeSummary.changed)}`);
    if (changeSummary.removed.length > 0)
      parts.push(`Removed: ${formatChangedNames(changeSummary.removed)}`);
    return parts.join(" · ");
  }, [changeSummary]);
  ...
  useEffect(() => {
    ...
    function handleDocumentClick(event: MouseEvent) {
      ...
      if (
        button.type === "submit" ||
        /\b(save|create|update|test|import)\b/i.test(label)
      ) {
        flushPendingDraft();
      }
    }
  ...
  function bulkImport(text: string, targetRowId: string): boolean {
    ...
    updateDraft(working);
    toast?.pushToast({
      title: `Imported ${pairs.length} variable${pairs.length === 1 ? "" : "s"}`,
      tone: "success",
    });
    return true;
  }
  ...
  const attentionCount = useMemo(
    () =>
      rows.reduce(
        (count, row) =>
          computeRowHealth(row, secrets) ||
          computeUserSecretRowHealth(row, userSecretDefinitions)
            ? count + 1
            : count,
        0,
      ),
  ...
  const quickBind = useMemo(() => {
    const boundIds = new Set(
      rows
        .filter((row) => row.source === "secret" && row.secretId)
        .map((row) => row.secretId),
    );
    return (recentlyUsedSecrets ?? [])
      .filter(
        (secret) => secret.status === "active" && !boundIds.has(secret.id),
      )
      .slice(0, 8);
  ...
  return (
    <TooltipProvider>
      <div ref={editorRootRef} className="@container/env space-y-2">
            ...
            {rows.map((row, index) => {
              ...
              return (
                <EnvironmentVariableRow
                  ...
                  dirtyFields={rowDirtyFields(
                    row,
                    committedRowsById.get(row.id),
                  )}
                  ...
                  onToast={(message) =>
                    toast?.pushToast({ title: message, tone: "success" })
                  }
                  focusRequest={
                    pendingFocus?.rowId === row.id ? pendingFocus.field : null
                  }
        ...
        {hasUnsavedChanges && !disabled ? (
          ...
          >
            <div className="flex min-w-0 flex-col gap-0.5">
              ...
              {changeSummaryText ? (
                <p
                  className="min-w-0 truncate pl-4 text-xs text-amber-950/80 dark:text-amber-100/80"
        ...
        {rows.some(
          (row) => row.source === "user_secret" && row.userSecretKey,
        ) ? (
          <p className="inline-flex items-start gap-1 text-(length:--text-micro) text-muted-foreground/70">
            ...
            <span>
              User secrets resolve from the user responsible for the run.
              Required bindings fail until that user sets their value under
              Secrets → My secrets.
            </span>
          </p>
        ) : null}
