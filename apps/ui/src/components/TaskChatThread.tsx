...
import {
  ...
  type PaperclipTaskRuntimeOwnerChange,
} from "../hooks/usePaperclipTaskRuntime";
...
import {
  type TaskTimelineOwner,
  type TaskTimelineEvent,
} from "../lib/task-timeline-events";
...
import { TaskOwnerBacklogNotice } from "./TaskOwnerBacklogNotice";
...
function TaskChatImmediateParentLabel({
  custom,
}: {
...
class TaskChatErrorBoundary extends Component<
...
function TaskOwnerPausedNotice({ agent }: { agent: Agent | null }) {
...
const TaskChatTextPart = memo(function TaskChatTextPart({
  text,
  recessed,
  onAccent,
}: {
...
function TaskChatRollingToolPart({
  toolParts,
}: {
...
const TaskChatTextParts = memo(function TaskChatTextParts({
  message,
  recessed = false,
  onAccent = false,
}: {
...
const TaskChatAssistantParts = memo(function TaskChatAssistantParts({
  message,
  hasCoT,
}: {
  message: ThreadMessage;
...
function TaskChatAssistantMessage({
  message,
  isRunActive,
  isStoppingRun,
}: {
          ...
          {isFoldable ? (
            ...
            <div className="mb-1.5 flex items-center gap-2">
              ...
              {isRunning ? (
                ...
                // task header's "Live" badge (one live/running blue).
                <Badge
                  variant="outline"
                  className={cn(
...
function isTaskCommentPresentation(
  value: unknown,
): value is TaskCommentPresentation {
  if (!value || typeof value !== "object") return false;
  const v = value as Record<string, unknown>;
  return v.kind === "system_notice" || v.kind === "message";
}

function isTaskCommentMetadata(value: unknown): value is TaskCommentMetadata {
  if (!value || typeof value !== "object") return false;
  const v = value as Record<string, unknown>;
...
function taskChatMessageCustom(
  message: ThreadMessage,
): Record<string, unknown> {
  return (message.metadata?.custom ?? {}) as Record<string, unknown>;
}

function taskChatMessageKind(message: ThreadMessage): string {
...
function taskChatMessageQueueTargetRunId(
  message: ThreadMessage,
): string | null {
  const custom = taskChatMessageCustom(message);
...
function taskChatMessageRunIsActive(
  ...
  const runId = taskChatMessageRunId(message);
  return Boolean(runId && activeRunIds.has(runId));
}

function taskChatMessageRunIsStopping(
  message: ThreadMessage,
...
function taskChatMessageQueuedRunIsInterrupting(
  message: ThreadMessage,
  interruptingQueuedRunId: string | null | undefined,
): boolean {
  const queueTargetRunId = taskChatMessageQueueTargetRunId(message);
  return Boolean(
...
interface VirtualizedTaskChatThreadListProps {
  messages: readonly ThreadMessage[];
  activeRunIds: ReadonlySet<string>;
  stoppingRunId?: string | null;
  interruptingQueuedRunId?: string | null;
...
interface VirtualizedTaskChatThreadListHandle {
  scrollToIndex: (
    index: number,
    options?: {
...
function taskChatMessageAnchorId(message: ThreadMessage): string | null {
...
function findMessageAnchorIndex(
  ...
  return messages.findIndex(
    (message) => taskChatMessageAnchorId(message) === anchorId,
  );
}
...
export function findLatestCommentMessageIndex(
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const anchorId = taskChatMessageAnchorId(messages[index]);
...
function useTaskThreadVirtualizer({
  count,
  estimateSize,
  overscan,
  scrollMargin,
...
// The chat thread renders inside `<main id="main-content">` on the real task
...
const VirtualizedTaskChatThreadList = forwardRef<
  VirtualizedTaskChatThreadListHandle,
  VirtualizedTaskChatThreadListProps
>(function VirtualizedTaskChatThreadList(props, ref) {
  ...
  return (
    <VirtualizedTaskChatThreadListInner
...
interface VirtualizedTaskChatThreadListInnerProps extends VirtualizedTaskChatThreadListProps {
  mode: VirtualizedScrollMode;
  probeRef: React.MutableRefObject<HTMLDivElement | null>;
}

const VirtualizedTaskChatThreadListInner = forwardRef<
  VirtualizedTaskChatThreadListHandle,
  VirtualizedTaskChatThreadListInnerProps
>(function VirtualizedTaskChatThreadListInner(
  {
    messages,
    activeRunIds,
    stoppingRunId,
  ...
  const virtualizer = useTaskThreadVirtualizer({
  ...
  return (
    <div
      ref={setRefs}
      data-testid="task-chat-thread-virtualizer"
      ...
      {virtualItems.map((virtualItem) => {
        ...
        return (
          <div
            ...
            data-testid="task-chat-thread-virtual-row"
            ref={(element) => {
...
interface TaskChatMessageRowProps {
  message: ThreadMessage;
  activeRunIds: ReadonlySet<string>;
  stoppingRunId?: string | null;
  interruptingQueuedRunId?: string | null;
...
function TaskChatCommentGroupContinuation({
  message,
}: {
  message: ThreadMessage;
}) {
...
const TaskChatMessageRow = memo(function TaskChatMessageRow({
  message,
  activeRunIds,
  stoppingRunId,
  interruptingQueuedRunId,
...
function areTaskChatMessageRowPropsEqual(
...
const TaskChatComposer = forwardRef<
...
export function TaskChatThread({
  ...
  const virtualizedThreadRef =
    useRef<VirtualizedTaskChatThreadListHandle | null>(null);
  ...
  // to the real bottom. The convergence loop below tasks `scrollIntoView`
  ...
  function scrollToLatestCommentWithSettle(
    ...
    const latestCommentAnchor = taskChatMessageAnchorId(
              ...
              >
                ...
                {showComposer ? (
                  ...
                  >
                    ...
                    />
                    <TaskOwnerPausedNotice agent={ownerAgent} />
                  </div>
                ) : null}
                {footer ? (
                  <div data-testid="task-chat-thread-footer">{footer}</div>
                ) : null}
