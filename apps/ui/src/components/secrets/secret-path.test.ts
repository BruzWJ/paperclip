...
describe("secret path hierarchy", () => {
  it("derives folder segments from slash-delimited stored names", () => {
    const rows = [
...
describe("buildSecretPathListing", () => {
  it("shows a name as both a direct secret and a folder when it is also a prefix", () => {
    ...
    expect(buildSecretPathListing([exact, nested], "dev")).toEqual({
      folders: [
        { name: "oauth", path: "dev/oauth", secretCount: 1, folderCount: 0 },
      ],
      secrets: [exact],
    });
  });
  ...
  it("groups case-sensitively while sorting case-insensitively", () => {
    ...
    expect(listing.secrets.map(getSecretPathRowName)).toEqual([
      "Alpha",
      "alpha",
      "zebra",
    ]);
  ...
  it("preserves encoded and special characters in segments and breadcrumbs", () => {
    ...
    expect(
      buildSecretPathBreadcrumbs("/team & ops//100%/api?key=a%2Fb/"),
    ).toEqual([
...
describe("validateSecretFolderSegment", () => {
  ...
  it("rejects padded aliases and accepts exact special-character segment names", () => {
    expect(validateSecretFolderSegment("  dev tools  ")).toBe(
      "Folder name must not contain surrounding whitespace.",
    );
