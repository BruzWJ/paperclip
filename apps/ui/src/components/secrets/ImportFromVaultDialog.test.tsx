...
describe("ImportFromVaultDialog", () => {
      ...
      .mockResolvedValueOnce(
        makePreview(
          [
            makeCandidate({
              externalRef:
                "arn:aws:secretsmanager:us-east-1:1:secret:prod/stripe-ABC",
            ...
            makeCandidate({
              externalRef:
                "arn:aws:secretsmanager:us-east-1:1:secret:prod/openai-XYZ",
      ...
      .mockResolvedValueOnce(
        makePreview(
          [
            makeCandidate({
              externalRef:
                "arn:aws:secretsmanager:us-east-1:1:secret:prod/sendgrid-Q9",
  ...
  it("blocks import when a review row collides with an existing Paperclip secret", async () => {
    ...
    expect(document.body.textContent?.toLowerCase()).toContain(
      "a paperclip secret already uses this",
    );
  ...
  it("renders mixed import results (created/skipped/failed) and shows error reason", async () => {
    ...
    expect(document.body.textContent).toContain(
      "AWS Secrets Manager denied the request",
    );
  ...
  it("shows an empty state when no AWS vault is configured", async () => {
    ...
    expect(
      document.querySelector('[data-testid="select-empty-vaults"]'),
    ).not.toBeNull();
