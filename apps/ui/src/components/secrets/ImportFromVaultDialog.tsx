...
function StatusBadge({
  ...
  return (
    <Badge
      variant="outline"
      className={cn("gap-1 px-1.5 py-0 font-normal", statusToneClasses(status))}
    >
...
function RowResultBadge({
  status,
}: {
  status: RemoteSecretImportRowResult["status"];
}) {
  switch (status) {
    case "imported":
      return (
...
export function ImportFromVaultDialog({
  ...
  const awsVaults = useMemo(
    () => awsVaultOptions(providerConfigs),
    [providerConfigs],
  );
  const eligible = useMemo(
    () => eligibleVaults(providerConfigs),
    [providerConfigs],
  );
  ...
  const [selection, setSelection] = useState<Map<string, DraftSelection>>(
    new Map(),
  );
  const [importResult, setImportResult] =
    useState<RemoteSecretImportResult | null>(null);
  ...
  const visibleCandidates = useMemo<RemoteSecretImportCandidate[]>(() => {
    ...
    return preview.candidates.filter((candidate) =>
      selection.has(candidate.externalRef),
    );
  ...
  const importMutation = useMutation({
    mutationFn: (input: RemoteImportInput) =>
      secretsApi.remoteImport(companyId, input),
    onSuccess: (result) => {
  ...
  function toggleAllLoaded() {
    setSelection((prev) => {
      const next = new Map(prev);
      const allSelected = selectableInLoaded.every((c) =>
        next.has(c.externalRef),
      );
      if (allSelected) {
  ...
  function handleSubmitImport() {
    ...
    const items: RemoteImportSelectionInput[] = draftList.map((draft) => ({
      ...
      providerMetadata: safeImportProviderMetadata(
        draft.candidate.providerMetadata,
      ),
    }));
  ...
  if (noEligibleVaults) {
    return (
      <div
        ...
        data-testid="select-empty-vaults"
      >
        <EmptyState
          icon={Cloud}
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        ...
        {awsVaults.length === 1 && eligible.length === 1 ? (
          <span
            ...
            data-testid="vault-static-label"
          >
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {vault.status === "warning" && (
                        <Badge
                          variant="outline"
                          className="px-1 py-0 text-(length:--text-nano) text-amber-500 border-amber-500/40"
                        >
                          warning
                        </Badge>
                      )}
                      {blocked && (
                        <Badge
                          variant="outline"
                          className="px-1 py-0 text-(length:--text-nano) text-muted-foreground"
                        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {selection.size} selected · {selectedNotVisible} not visible with
            current search
          </span>
          <Button
            variant="ghost"
      ...
      <div
        ...
        data-testid="vault-table-scroll"
      >
        {previewError ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody data-testid="vault-table-body">
              {visibleCandidates.map((candidate) => {
                ...
                const meta = (candidate.providerMetadata ?? {}) as Record<
                  string,
                  unknown
                >;
                const lastChanged =
...
function EmptyCandidates({ query }: { query: string }) {
  ...
  return (
    <EmptyState icon={Database} message="No secrets visible to this vault." />
  );
}
