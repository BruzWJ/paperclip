...
import type { AttentionSourceKind } from "@paperclipai/shared";
...
import {
  ...
  noop,
  render,
} from "./AttentionQueueRow.test-support";
describe("AttentionQueueRow", () => {
  it("renders an inline approval resolver when expanded", () => {
    const el = render(
      <AttentionQueueRow item={buildItem()} companyId="c1" expanded onToggleExpand={noop} onDismiss={noop} />,
    );
  ...
  it("uses the shadcn item surface and can show a keyboard selection ring", () => {
    render(
      <AttentionQueueRow
        item={buildItem()}
        companyId="c1"
  ...
  it("submits a compact approval without expanding the card and confirms it", async () => {
    ...
    expect(toast.getHistory().some((item) => "title" in item && item.title === "Approval approved")).toBe(
      true,
    );
  });
  it("renders evidence thumbnails in a centered context row below the text stack", () => {
    ...
    const image = container?.querySelector('img[alt="Visual evidence attachment"][aria-label="Screenshot"]');
  ...
  it("does not expose a toggle button for non-inline rows", () => {
    render(
      <AttentionQueueRow
        item={buildItem({
          sourceKind: "mention_board" as AttentionSourceKind,
  ...
  it("shows a larger gallery with an n-more link to the task when expanded", () => {
    ...
    expect(moreLink?.getAttribute("href")).toBe("/11111111-1111-4111-8111-111111111111/tasks/42");
  });
