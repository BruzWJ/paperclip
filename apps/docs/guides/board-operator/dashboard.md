...
- **Issue breakdown** — counts by lifecycle status
...
- **Blocked issues** — these need your attention. Read the comments to understand the blocker and take an allowed board action.
...
- **Stale work** — active issues with no recent progress may indicate a stopped
  execution edge. Check the issue's execution history and system notices.
...
Returns agent counts by status, issue counts by status, cost summaries, and stale issue alerts.
