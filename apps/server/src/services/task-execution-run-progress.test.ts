...
import {
  clearAllTaskExecutionRunProgresses,
  clearTaskExecutionRunProgress,
  getTaskExecutionRunProgress,
  MAX_TASK_EXECUTION_RUN_PROGRESS_MESSAGE_CHARS,
  setTaskExecutionRunProgress,
  sweepExpiredTaskExecutionRunProgresses,
  touchTaskExecutionRunProgress,
} from "./task-execution-run-progress.js";

describe("task execution run progress store", () => {
  afterEach(() => {
    clearAllTaskExecutionRunProgresses();
  });
  ...
  it("stores scoped ephemeral status and expires stale entries", () => {
    ...
    const status = setTaskExecutionRunProgress({
      companyId: "company-1",
      taskId: "task-1",
      agentId: "agent-1",
      runId: "run-1",
    ...
    expect(status?.message.length).toBeLessThanOrEqual(MAX_TASK_EXECUTION_RUN_PROGRESS_MESSAGE_CHARS);
    ...
    expect(getTaskExecutionRunProgress("run-1", {
      companyId: "company-1",
      taskId: "task-1",
      agentId: "agent-1",
    ...
    expect(getTaskExecutionRunProgress("run-1", { companyId: "other-company" })).toBeNull();
    expect(getTaskExecutionRunProgress("run-1", {
      companyId: "company-1",
      now: new Date("2026-06-24T00:02:00.001Z"),
    })).toBeNull();
    expect(getTaskExecutionRunProgress("run-1")).toBeNull();
  });
  ...
  it("clears status explicitly", () => {
    ...
    expect(clearTaskExecutionRunProgress("run-1")).toBe(true);
    expect(getTaskExecutionRunProgress("run-1")).toBeNull();
  });
  ...
  it("touch refreshes timestamps while preserving the existing status context", () => {
    ...
    const touched = touchTaskExecutionRunProgress({
      companyId: "company-1",
      taskId: "task-1",
      agentId: "agent-1",
      runId: "run-1",
  ...
  it("touch creates a fallback run_activity status when none is live", () => {
    const created = touchTaskExecutionRunProgress({
      companyId: "company-1",
      taskId: "task-1",
      agentId: "agent-1",
      runId: "run-1",
    ...
    const expiredTouch = touchTaskExecutionRunProgress({
      companyId: "company-1",
      taskId: "task-1",
      agentId: "agent-1",
      runId: "run-1",
  ...
  it("sweeps expired statuses without touching fresh entries", () => {
    ...
    expect(sweepExpiredTaskExecutionRunProgresses(now)).toBe(1);
    expect(getTaskExecutionRunProgress("stale-run")).toBeNull();
    expect(getTaskExecutionRunProgress("fresh-run", { now })).toMatchObject({ runId: "fresh-run" });
  });
});
