...
import {
  ...
  type InvokableTaskOwnerResolution,
...
export type PluginTaskOwnerOperation = "tasks.create" | "tasks.update";

export interface PluginTaskAuthorizationIdentity {
  companyId: string;
...
export class PluginTaskAuthorizationRejected extends Error {
  readonly code = "plugin_task_authorization_rejected";

  constructor(
    message: string,
    readonly reason: PluginTaskAuthorizationRejectionReason,
    readonly details: Record<string, unknown>,
  ) {
    super(message);
    this.name = "PluginTaskAuthorizationRejected";
  }
}

function reject(
  reason: PluginTaskAuthorizationRejectionReason,
  message: string,
  details: Record<string, unknown>,
): never {
  throw new PluginTaskAuthorizationRejected(message, reason, details);
}

function assertInstallationRequestScope(
  input: PluginTaskAuthorizationIdentity & PluginInstallationCompanyLockScope,
...
function assertPluginTaskOperationAvailability(
  input: PluginTaskAuthorizationIdentity &
 ...
 */
export function resolvePluginPermittedTaskOwnerCatalog(
  input: PluginTaskOwnerCatalogInput,
): ReadonlyMap<string, InvokableTaskOwnerResolution> {
  assertPluginTaskOperationAvailability(input);
  return new Map(input.invokableOwnerCatalog);
}

export function selectPluginPermittedTaskOwner(
  catalog: ReadonlyMap<string, InvokableTaskOwnerResolution>,
  input: PluginTaskAuthorizationIdentity & {
    operation: PluginTaskOwnerOperation;
    ownerAgentId: string;
  },
  ...
  if (!owner) {
    reject(
      "plugin_owner_not_permitted",
      "Requested owner is not in the plugin-permitted task owner catalog",
      {
        companyId: input.companyId,
        pluginInstallationId: input.pluginInstallationId,
        operation: input.operation,
 ...
 */
export async function resolvePluginPermittedTaskOwnerCatalogInTransaction(
 ...
 */
export async function assertPluginPermittedTaskOwnerInTransaction(
  tx: TaskSessionDbTransaction,
  input: PluginTaskAuthorizationIdentity & {
    operation: PluginTaskOwnerOperation;
    ownerAgentId: string;
