...
const contextDial = {
  carry_context: true,
  read_issue_comments: false, read_issue_agent_run: false,
  list_sub_issues: false, read_sub_issue_comments: false,
  read_sub_issue_agent_run: false, list_company_issues: false,
  read_company_issue_comments: false, read_company_issue_agent_run: false,
} as const;
...
function operationScenario(input: {
  ...
  ref?: Record<string, unknown>; carry?: boolean; correlation?: boolean;
  instruction?: string | null;
}) {
  const ref = sessionRef(input.kind === "instruction"
    ? { messageKind: "synthetic" }
    : input.kind === "failed_bootstrap"
      ? { id: "ref-2", laneOrdinal: 1 }
      : input.ref);
  const grouped = input.kind === "instruction"
    ? [ref, sessionRef({ id: "ref-2", laneOrdinal: 1 })]
    : input.kind === "failed_bootstrap"
      ? [sessionRef({ messageKind: "synthetic", disposition: "terminal" }), ref]
      : [ref];
  const tail = input.kind === "failed_bootstrap"
    ? [[{ runId: "bootstrap-run", refOrdinal: 0, outcome: "failed", protocolSettlementState: "incomplete", correlation: null }]]
    : [
        [{ instruction: input.instruction ?? null }],
        ...(input.carry === false
          ? []
          : [[...(input.correlation ? [{ id: "carry-1" }] : [])]]),
      ];
...
describe("base prompt ACPX session operation", () => {
  it.each([
    ...
    ["instructionless reassignment without carry", {
      kind: "ordinary", carry: false,
      ref: { ownershipEpoch: 2, previousOwnershipEpoch: 1, sourceKind: "issue_reassignment" },
    }, "new"],
  ...
  it.each([
    ["failed bootstrap", { kind: "failed_bootstrap" }, "ordered session-start work lost its exact bootstrap correlation"],
    ["instructed work without carry", { kind: "ordinary", instruction: "Lead delivery.", ref: { laneOrdinal: 2 } }, "instructed work lost its exact carry or ordered session start"],
    ["instructed reassignment singleton", { kind: "ordinary", carry: false, instruction: "Lead delivery.", ref: { ownershipEpoch: 2, previousOwnershipEpoch: 1, sourceKind: "issue_reassignment" } }, "instructed work lost its exact carry or ordered session start"],
