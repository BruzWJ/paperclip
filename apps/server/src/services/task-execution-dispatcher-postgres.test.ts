...
const contextDial = {
  carry_context: true,
  read_task_comments: false, read_task_agent_run: false,
  list_sub_tasks: false, read_sub_task_comments: false,
  read_sub_task_agent_run: false, list_company_tasks: false,
  read_company_task_comments: false, read_company_task_agent_run: false,
} as const;
...
describe("base prompt ACPX session operation", () => {
  ...
  it.each([
    ...
    ["instructed reassignment singleton", { kind: "ordinary", carry: false, instruction: "Lead delivery.", ref: { ownershipEpoch: 2, previousOwnershipEpoch: 1, sourceKind: "task_reassignment" } }, "instructed work lost its exact carry or ordered session start"],
