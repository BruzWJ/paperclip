...
function promptIdentity(): TaskExecutionPromptIdentity {
  return {
...
function executionRef(
  ...
  return {
    ...
    companyId: identity.companyId, taskId: identity.taskId,
    ...
    mode: identity.laneKind, sourceKind: "task_request",
    sourceRecordId: identity.taskId, messageKind: "user",
    ...
    contextEpoch: 0, counterpartTaskId: null,
...
function renewalTransaction(input: {
  ...
  const completedTransaction = Object.assign(transaction, { update }) as
    unknown as TaskSessionDbTransaction;
  return {
...
function leaseRow(identity: TaskExecutionPromptIdentity): TaskExecutionLease {
  return {
    id: identity.leaseId,
    companyId: identity.companyId,
    taskId: identity.taskId,
...
function renewalRepository(input: {
  ...
  const database = {
    transaction: vi.fn(
      async (work: (tx: TaskSessionDbTransaction) => unknown) =>
...
describe("Postgres task-execution prompt-cycle continuity fence", () => {
  ...
  it("rejects activation when a prepared generation does not clear the latest locked reopen fence", async () => {
    ...
    const database = {
      transaction: vi.fn(async (work: (tx: TaskSessionDbTransaction) => unknown) =>
        work(transaction)),
    } as unknown as Db;
    ...
    } as unknown as ResolvedTaskExecutionPrompt;
...
describe("Postgres prompt closure lock order", () => {
  it("locks company, task, and Session before the canonical run", async () => {
    ...
    await expect(repository.closePrompt({
      prompt: { identity: promptIdentity() } as ResolvedTaskExecutionPrompt,
      capability: {} as never,
      outcome: {} as never,
    })).rejects.toBe(stop);
...
describe("Postgres task-execution prompt authority renewal", () => {
  it("awaits each canonical row lock before requesting the next lock", async () => {
    ...
    const renewal = repository.renewPromptAuthority({
      identity,
    } as ResolvedTaskExecutionPrompt);
    ...
    gates.get(taskExecutionPromptCapabilities)!.resolve([
