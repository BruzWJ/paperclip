...
const TASK_DISPLAY_LABEL_MAX_CHARS = 96;
...
function taskDisplayLabel(task: Pick<TaskSearchRow, "id" | "identifier" | "title" | "request">) {
...
function emptySearchCounts(): Record<CompanySearchCountType, number> {
  return { task: 0, comment: 0, document: 0, artifact: 0, agent: 0, project: 0 };
}
...
function taskOnlyFiltersActive(query: CompanySearchQuery) {
...
function queryWithoutFilter(query: CompanySearchQuery, key: CompanySearchTaskFilterKey): CompanySearchQuery {
  return {
    ...query,
...
function queryWithoutTaskFilters(query: CompanySearchQuery): CompanySearchQuery {
  return {
    ...query,
    status: [],
    priority: [],
...
function taskFilterConditions(companyId: string, query: CompanySearchQuery, omit?: CompanySearchTaskFilterKey): SQL[] {
  const conditions: SQL[] = [];
  if (omit !== "status" && query.status.length > 0) {
    conditions.push(query.status.length === 1 ? eq(tasks.boardPresentationStatus, query.status[0]!) : inArray(tasks.boardPresentationStatus, query.status));
  }
...
function matchedFacetConditions(companyId: string, query: CompanySearchQuery, omit?: CompanySearchTaskFilterKey): SQL[] {
  const conditions: SQL[] = [];
  if (omit !== "status" && query.status.length > 0) {
...
function scopeIncludesTasks(scope: CompanySearchScope) {
...
function selectPrimarySnippets(row: TaskSearchRow, normalizedQuery: string, tokens: string[]) {
...
function taskResult(row: TaskSearchRow, prefix: string, normalizedQuery: string, tokens: string[]): CompanySearchResult {
  ...
  const task: CompanySearchTaskSummary = {
    ...
    boardPresentationStatus:
      row.status as CompanySearchTaskSummary["boardPresentationStatus"],
...
function artifactResult(artifact: CompanyArtifact, normalizedQuery: string, tokens: string[]): CompanySearchResult {
  ...
  const score = scoreSimpleRow({
    ...
    description: [artifact.previewText, artifact.task.identifier, artifact.task.title, artifact.project?.name]
      .filter(Boolean)
      .join(" "),
...
export function companySearchService(db: Db) {
  ...
  return {
    ...
    search: async (companyId: string, query: CompanySearchQuery): Promise<CompanySearchResponse> => {
      ...
      if (!hasSearchText && !taskOnlyFiltersActive(query)) {
        return {
          query: query.q,
          normalizedQuery,
          scope,
      ...
      const titlePhraseMatch = hasSearchText ? sql<boolean>`tasks.title ILIKE ${containsPattern}` : noMatchSql();
      const titleStartsWith = hasSearchText ? sql<boolean>`tasks.title ILIKE ${startsWithPattern}` : noMatchSql();
      const titleExactMatch = hasSearchText ? sql<boolean>`lower(tasks.title) = ${normalizedQuery}` : noMatchSql();
      const identifierPhraseMatch = hasSearchText ? sql<boolean>`coalesce(tasks.identifier, '') ILIKE ${containsPattern}` : noMatchSql();
      const identifierStartsWith = hasSearchText ? sql<boolean>`coalesce(tasks.identifier, '') ILIKE ${startsWithPattern}` : noMatchSql();
      const identifierExactMatch = hasSearchText ? sql<boolean>`lower(coalesce(tasks.identifier, '')) = ${normalizedQuery}` : noMatchSql();
      const requestPhraseMatch = hasSearchText ? sql<boolean>`coalesce(tasks.request, '') ILIKE ${containsPattern}` : noMatchSql();
      const titleTokenMatch = tokenCount > 0 ? sql<boolean>`tasks.title ILIKE ANY(${tokenPatternArray})` : noMatchSql();
      const identifierTokenMatch = tokenCount > 0 ? sql<boolean>`coalesce(tasks.identifier, '') ILIKE ANY(${tokenPatternArray})` : noMatchSql();
      const requestTokenMatch = tokenCount > 0 ? sql<boolean>`coalesce(tasks.request, '') ILIKE ANY(${tokenPatternArray})` : noMatchSql();
      ...
      const fuzzyIdentifierMatch = fuzzyEnabled
        ? sql<boolean>`similarity(lower(coalesce(tasks.identifier, '')), ${normalizedQuery}) >= ${FUZZY_IDENTIFIER_SIMILARITY_THRESHOLD}`
        : noMatchSql();
      ...
      const taskFilters = taskFilterConditions(companyId, query);
      const hasTaskOnlyFilters = taskOnlyFiltersActive(query);
      ...
      type TaskAggregates = {
        ...
        totals: { current: number; unfiltered: number; omit: Partial<Record<CompanySearchTaskFilterKey, number>> };
      };
      type TaskSearchData = { rows: TaskSearchRow[]; aggregates: TaskAggregates };
      ...
      async function fetchTaskSearchData(): Promise<TaskSearchData> {
        ...
        if (scope === "all" || scope === "tasks") {
          branches.push(sql`SELECT 'type:task' AS kind, NULL::text AS value, count(*)::int AS count ${countTail} FROM matched m ${branchWhere([...facetsAll, titleCond])}`);
        }
        ...
        const facetBranch = (kind: string, valueSql: SQL, omit: CompanySearchTaskFilterKey, extra: SQL[] = []) => sql`
        ...
        `) as unknown as Array<SearchAggregateRow & Omit<TaskSearchRow, "commentSnippet" | "commentId" | "documentSnippet" | "documentTitle" | "documentKey">>;
        ...
        const taskRowsRaw: TaskSearchRow[] = [];
        for (const row of resultRows) {
          ...
          else if (row.kind === "facet:status" && row.value && (TASK_STATUSES as readonly string[]).includes(row.value)) {
            aggregates.filterOptionCounts.status[row.value as keyof CompanySearchFilterOptionCounts["status"]] = count;
          } else if (row.kind === "facet:priority" && row.value && (TASK_PRIORITIES as readonly string[]).includes(row.value)) {
        ...
        return { rows: await enrichTaskSnippets(taskRowsRaw), aggregates };
      }
      ...
      async function enrichTaskSnippets(rows: TaskSearchRow[]): Promise<TaskSearchRow[]> {
      ...
      async function countArtifacts(filters: CompanySearchQuery = query) {
        ...
        const [documentRows, workProductRows, attachmentRows] = await Promise.all([
          db
            ...
            .from(taskDocuments)
            .innerJoin(documents, and(eq(taskDocuments.documentId, documents.id), eq(documents.companyId, taskDocuments.companyId)))
          ...
          db
            ...
            .innerJoin(tasks, and(eq(taskWorkProducts.taskId, tasks.id), eq(tasks.companyId, taskWorkProducts.companyId)))
            .where(and(...workProductConditions)),
          db
            .select({ count: sql<number>`count(*)::int` })
            .from(taskAttachments)
            .innerJoin(assets, and(eq(taskAttachments.assetId, assets.id), eq(assets.companyId, taskAttachments.companyId)))
            .innerJoin(tasks, and(eq(taskAttachments.taskId, tasks.id), eq(tasks.companyId, taskAttachments.companyId)))
      ...
      const currentTotalCount = (scopeIncludesTasks(scope) ? aggregates.totals.current : 0)
        + artifactCount
        + agentCount
        + projectCount;
      ...
      const zeroResults = currentTotalCount === 0 && filtersActive.length > 0
        ? {
          unfilteredTotal: (scopeIncludesTasks(scope) ? aggregates.totals.unfiltered : 0)
            + await countTotalNonTask(queryWithoutTaskFilters(query)),
          loosenSuggestions: (await Promise.all(filtersActive.map(async (filter) => {
            const resultCount = (scopeIncludesTasks(scope) ? aggregates.totals.omit[filter.key] ?? 0 : 0)
