...
export function resolvePluginPath(
  ...
  if (!path.isAbsolute(root)) {
    throw new PluginPathError(
      "invalid_root",
      `${options.label} root must be absolute`,
    );
  }
  if (!relativePath || path.isAbsolute(relativePath)) {
    throw new PluginPathError(
      "invalid_relative_path",
      `${options.label} must be relative to its root`,
    );
  }
  ...
  if (!fs.statSync(canonicalRoot).isDirectory()) {
    throw new PluginPathError(
      "invalid_root",
      `${options.label} root is not a directory`,
    );
  }
  ...
  if (!isPathContained(canonicalRoot, candidate)) {
    throw new PluginPathError(
      "escape",
      `${options.label} must resolve inside its root`,
    );
  }
  ...
  if (
    (options.kind === "file" && !stat.isFile())
    || (options.kind === "directory" && !stat.isDirectory())
  ) {
    throw new PluginPathError(
      "wrong_kind",
      `${options.label} is not a ${options.kind}`,
    );
  }
