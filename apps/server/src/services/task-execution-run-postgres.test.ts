...
const request: RequestedTaskExecutionSteering = {
  companyId: "company", taskId: "task", ownershipEpoch: 1,
...
function steeringAdmissionPlan(now: Date, message: string) {
  return {
    select: rows(
      {
        id: "run", companyId: "company", taskId: "task",
        ...
        taskExecutionAuthorityId: "authority", consultExecutionId: null,
...
describe("PostgreSQL task-execution steering recovery", () => {
  it("admits steering while the base prompt segment is still open", async () => {
    ...
    const repository = createPostgresTaskExecutionSteeringRepository(
      harness.db,
      ...
      select: rows(
        {
          ...
          taskId: request.taskId, runId: request.runId, state: intentState,
        },
        {
