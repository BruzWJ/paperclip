 ...
 */
export function classifyOrderedExecutionScopePair<Member extends OrderedExecutionScopeMember>(
  refs: readonly Member[],
): { readonly instruction: Member; readonly work: Member } | null {
  if (refs.length !== 2) return null;
  const [instruction, work] =
    refs[0]!.laneOrdinal < refs[1]!.laneOrdinal ? [refs[0]!, refs[1]!] : [refs[1]!, refs[0]!];
