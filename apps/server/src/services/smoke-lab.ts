...
// with cwd=<repo>/apps/server, so a cwd-relative path points at
// <repo>/apps/server/scripts/... —
// which does not exist — and the spawned sidecar exits code=1 ("Cannot find module").
// This module sits at apps/server/src/services (and apps/server/dist/services in a build), so the
// repo root is four parent traversals up in both layouts. A cwd fallback keeps other layouts working.
