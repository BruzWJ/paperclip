...
function issueListRow(overrides: Record<string, unknown> = {}) {
  return {
    id: rootId,
    companyId,
    projectId: null,
...
function currentRunLinkage(issueId: string) {
  return {
    ...
    issueExecutionAuthorityId: "00000000-0000-4000-8000-00000000060b",
...
async function classify(input: {
  ...
  const rows = await issueService(harness.db).list(companyId, {
    status: "blocked",
  });
...
describe("issue blocker attention", () => {
  ...
  it("rejects malformed blocked-inbox owner filters before database access", async () => {
    ...
    const svc = issueService(harness.db);
