...
/**
 * Regression test for paperclipai/paperclip#4628.
 *
 * Stands up a minimal Express app whose handler mirrors the parsing path
 * `apps/server/src/routes/tasks.ts:957-958` uses to forward `req.query.status`
 * into `taskService.list({ status })`. Verifies the four shapes Express's
...
function buildApp() {
  ...
  app.get("/api/companies/:companyId/tasks", (req, res) => {
    // Mirror the cast at routes/tasks.ts:958 exactly. Pre-fix this said
...
describe("task list status query parsing", () => {
  it("accepts a single ?status=todo and returns one normalized status", async () => {
    const res = await request(buildApp()).get("/api/companies/c1/tasks?status=todo");
  ...
  it("accepts comma-separated ?status=todo,in_progress (preserves legacy CSV)", async () => {
    const res = await request(buildApp()).get(
      "/api/companies/c1/tasks?status=todo,in_progress",
    );
    expect(res.status).toBe(200);
  ...
  it("accepts repeated ?status=todo&status=in_progress without crashing (the bug fix)", async () => {
    const res = await request(buildApp()).get(
      "/api/companies/c1/tasks?status=todo&status=in_progress",
    );
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ statuses: ["todo", "in_progress"] });
  });
  ...
  it("accepts mixed array + CSV ?status=todo,in_progress&status=done", async () => {
    const res = await request(buildApp()).get(
      "/api/companies/c1/tasks?status=todo,in_progress&status=done",
    );
    expect(res.status).toBe(200);
  ...
  it("accepts no ?status param and returns an empty status filter", async () => {
    const res = await request(buildApp()).get("/api/companies/c1/tasks");
    expect(res.status).toBe(200);
