...
describe("document annotation routes", () => {
  beforeEach(() => {
    ...
    mockAnnotationService.listThreadsForTaskDocument.mockImplementation(
      async (
        _taskId: string,
        _key: string,
    ...
    mockAnnotationService.getThreadForTaskDocument.mockResolvedValue({
      ...annotationThread,
      comments: [annotationComment],
    });
    mockAnnotationService.createThread.mockResolvedValue({
      ...annotationThread,
      comments: [annotationComment],
    });
    mockAnnotationService.addComment.mockResolvedValue(annotationComment);
    mockAnnotationService.updateThread.mockResolvedValue({
      ...annotationThread,
      status: "resolved",
    });
  ...
  it("includes compact open annotations without comment bodies when a board read explicitly requests them", async () => {
    ...
    expect(
      mockAnnotationService.listThreadsForTaskDocument,
    ).toHaveBeenCalledWith(taskId, "plan", {
      status: "open",
  ...
  it("includes annotation comment bodies on board document reads only when both expansions are requested", async () => {
    const res = await request(await createApp())
      .get(
        `/api/tasks/${taskId}/documents/plan?includeAnnotations=true&includeAnnotationComments=true`,
      )
      .expect(200);

    expect(res.body.annotations[0].comments[0].body).toBe(
      "Please review PAP-1",
    );
    expect(
      mockAnnotationService.listThreadsForTaskDocument,
  ...
  it("rejects noncanonical annotation status instead of treating it as open", async () => {
    await request(await createApp())
      .get(`/api/tasks/${taskId}/documents/plan/annotations?status=OPEN`)
      .expect(422);
  ...
  it("adds annotation comments through the canonical annotation owner and resolves threads", async () => {
    ...
    await request(await createApp())
      .post(
        `/api/tasks/${taskId}/documents/plan/annotations/${annotationThread.id}/comments`,
      )
    ...
    const resolved = await request(await createApp())
      .patch(
        `/api/tasks/${taskId}/documents/plan/annotations/${annotationThread.id}`,
      )
