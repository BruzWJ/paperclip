...
import { taskIngressRoutes } from "../routes/task-ingress.js";
...
describe("canonical board task ingress", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("creates through OrdinaryTaskRuntime and dispatches its persisted ref", async () => {
  ...
  it("passes the selected project codebase through canonical creation", async () => {
    ...
    await request(app)
      .post(`/api/companies/${companyId}/tasks`)
      .send({
        request: "Run in the selected project directory",
        ownerAgentId,
    ...
    expect(create).toHaveBeenCalledWith(
      expect.objectContaining({ projectId, projectWorkspaceId }),
    );
  });
  ...
  it("requires an owner and rejects legacy create payloads instead of stripping them", async () => {
    ...
    await request(app)
      .post(`/api/companies/${companyId}/tasks`)
      .send({
        request: "Per-task context override is forbidden",
        ownerAgentId,
  ...
  it("replays canonical idempotent creation with HTTP 200", async () => {
    ...
    expect(retry.body).toMatchObject({ id: taskId, retried: true });
  });
  ...
  it("creates a board child with the path parent and the same named creator", async () => {
    ...
    await request(app)
      .post(`/api/tasks/${parentId}/children`)
      .send({
