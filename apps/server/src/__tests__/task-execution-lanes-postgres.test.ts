...
vi.mock("../services/task-execution-run-service.js", async (importActual) => ({
  ...(await importActual<typeof import("../services/task-execution-run-service.js")>()),
  readActiveTaskExecutionRefRunAvailability: mocks.readAvailability,
}));
...
function lease(ref: TaskExecutionRef): LeasedTaskExecutionRef {
  return {
    ref,
...
describe("task-execution target lanes", () => {
  ...
  it("serializes one target FIFO while a distinct target lane overlaps", async () => {
    ...
    const queues = new Map<string, LeasedTaskExecutionRef[]>([
    ...
    const repository: TaskExecutionDispatcherRepository = {
      ...
      async leaseNextRef(input: { lane: TaskExecutionTargetLaneIdentity }) {
  ...
  it("rejects invalid lease timing and inexact ref identities before querying", async () => {
    const harness = createMockDb();
    expect(() => createPostgresTaskExecutionDispatcherRepository({
      ...repositoryOptions(harness.db),
      leaseTtlMs: 999,
    })).toThrow(PostgresTaskExecutionDispatchRejected);
    const repository = createPostgresTaskExecutionDispatcherRepository(
