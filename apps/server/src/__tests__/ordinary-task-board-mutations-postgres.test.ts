...
vi.mock("../services/task-session/admission.js", async (importActual) => {
  ...
  return {
    ...actual,
    createTaskSessionAdmissionService: vi.fn(() => mocks.sessions),
  };
});

vi.mock("../services/runtime-task-action-port.js", async (importActual) => {
  const actual = await importActual<
    typeof import("../services/runtime-task-action-port.js")
  >();
  return {
    ...actual,
    createTaskFormCommitRuntime: vi.fn(() => mocks.taskForms),
  };
});
...
vi.mock("../services/agent-invokability.js", async (importActual) => {
  ...
  return {
    ...actual,
    resolveInvokableTaskOwnerInTransaction: mocks.resolveInvokableOwner,
  };
});
...
describe("ordinary task board mutations without a database", () => {
  ...
  it("persists a terminal-task comment without dispatching from prose", async () => {
    ...
    const comment = { id: "comment-1", taskId: TASK_ID, runId: null };
  ...
  it("delegates creator-form admission and translates a denied authority", async () => {
    ...
    const accepted = { task: { id: TASK_ID }, delivery: { id: "delivery-1" } };
    ...
    await expect(
      runtime.commitCreatorFormUpdate(TASK_ID, "Denied", authority),
    ).rejects.toEqual(
      expect.objectContaining<Partial<OrdinaryTaskRuntimeRejected>>({
