...
vi.mock("../services/budgets.js", async () => ({
  ...await vi.importActual<typeof import("../services/budgets.js")>("../services/budgets.js"),
...
describe("attention service", () => {
  ...
  it("returns ranked Board Attention rows across approvals, joins, reviews, budgets, and agent requests", async () => {
    ...
    const feed = await attentionService(harness.db).list(companyId, { userId: "board-user" });
  ...
  it("uses attention-prefixed dismissal keys and resurfaces newer activity", async () => {
    ...
    await expect(attentionService(hiddenHarness.db).list(companyId, { userId: "board-user" }))
      .resolves.toMatchObject({ totalCount: 0 });
    const included = await attentionService(includedHarness.db).list(companyId, {
      userId: "board-user",
    ...
    const resurfaced = await attentionService(resurfacedHarness.db).list(companyId, { userId: "board-user" });
  ...
  it("serves board users and rejects agent callers without database infrastructure", async () => {
    ...
    const boardResponse = await request(createApp(boardHarness.db, boardActor()))
      .get(`/api/companies/${companyId}/attention`);
    const agentResponse = await request(createApp(agentHarness.db, agent))
      .get(`/api/companies/${companyId}/attention`);
