...
} from "../services/issue-execution-attempt-retry-schedule-postgres.js";
...
function runRow(
  overrides: Partial<IssueExecutionRun> = {},
): IssueExecutionRun {
  return {
    id: "run-1",
    companyId: "company-1",
...
function attemptRow(
  overrides: Partial<IssueExecutionAttempt> = {},
...
function scheduleRow(
  overrides: Partial<IssueExecutionAttemptRetrySchedule> = {},
): IssueExecutionAttemptRetrySchedule {
  return {
    id: "schedule-1",
    companyId: "company-1",
    issueId: "issue-1",
...
function createTransaction(input: {
  run: IssueExecutionRun;
  predecessor: IssueExecutionAttempt;
  schedule?: IssueExecutionAttemptRetrySchedule | null;
  ...
  const transaction = {
    select(selection?: Record<string, unknown>) {
      ...
      const builder = {
        ...
        async for() {
          if (table === issueExecutionRuns) return [state.run];
          if (table === issueExecutionAttemptRetrySchedules) {
        ...
        values(inputValues: Record<string, unknown>) {
          ...
          return {
            async returning() {
              ...
              if (table === issueExecutionAttemptRetrySchedules) {
                state.schedule = values as unknown as IssueExecutionAttemptRetrySchedule;
                return [state.schedule];
              }
              if (table === issueExecutionAttempts) {
                const successor = values as unknown as IssueExecutionAttempt;
    ...
    update(table: unknown) {
      ...
      const builder = {
        ...
        async returning() {
          ...
          if (table === issueExecutionRuns) {
            state.run = { ...state.run, ...values } as IssueExecutionRun;
            return [state.run];
          }
          if (table === issueExecutionAttemptRetrySchedules && state.schedule) {
            state.schedule = {
              ...state.schedule,
              ...values,
            } as IssueExecutionAttemptRetrySchedule;
            return [state.schedule];
          }
          return [];
        },
  ...
  return {
    transaction: transaction as unknown as Parameters<
      typeof scheduleIssueExecutionAttemptRetryInTransaction
    >[0],
    state,
  };
}
...
describe("canonical PostgreSQL attempt retry schedules", () => {
  it("schedules only one detached terminal pre-send predecessor", async () => {
    ...
    const schedule = await scheduleIssueExecutionAttemptRetryInTransaction(
      harness.transaction,
      {
        id: "schedule-1",
        companyId: "company-1",
  ...
  it("rejects a transmitted predecessor before creating a schedule", async () => {
    ...
    await expect(
      scheduleIssueExecutionAttemptRetryInTransaction(harness.transaction, {
        id: "schedule-1",
        companyId: "company-1",
        issueId: "issue-1",
        runId: "run-1",
  ...
  it("fails closed before revalidation when a schedule is not due", async () => {
    ...
    await expect(
      claimIssueExecutionAttemptRetryInTransaction(harness.transaction, {
        companyId: "company-1",
        issueId: "issue-1",
        runId: "run-1",
        scheduleId: "schedule-1",
