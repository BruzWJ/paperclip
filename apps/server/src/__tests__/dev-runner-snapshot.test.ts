...
function createSnapshotOptions(root: string) {
  return {
    repoRoot: root,
    watchedDirectories: [path.join(root, "apps", "server")],
...
describe("dev-runner watched snapshot", () => {
  it("skips files that disappear between directory listing and stat", () => {
    ...
    expect(snapshot.has("apps/server/index.ts")).toBe(false);
  });
  ...
  it("skips directories that disappear before they can be read", () => {
    ...
    expect(snapshot.has("apps/server/routes/health.ts")).toBe(false);
  });
  ...
  it("returns null for missing file signatures and not-directory races", () => {
    ...
    const missingPath = path.join(root, "apps", "server", "deleted.ts");
    const fileSystem = {
      statSync() {
  ...
  it("reports deleted paths in diffs", () => {
    expect(diffSnapshots(new Map([["apps/server/deleted.ts", "1:1"]]), new Map())).toEqual(["apps/server/deleted.ts"]);
  });
});
