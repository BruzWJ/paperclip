...
describe("shouldTrackDevServerPath", () => {
  it("ignores generated state, diagnostic reports, and common test file paths", () => {
    expect(
      shouldTrackDevServerPath(
        ".paperclip/worktrees/PAP-712-project-configuration/.agents/runtime/state",
      ),
    ).toBe(false);
    expect(shouldTrackDevServerPath("apps/server/report.20260416.154629.4965.0.001.json")).toBe(false);
    expect(shouldTrackDevServerPath("apps/server/report.20260416.154636.4725.0.001.json")).toBe(false);
    expect(shouldTrackDevServerPath("apps/server/report.20260416.154636.4965.0.002.json")).toBe(false);
    expect(shouldTrackDevServerPath("apps/server/src/__tests__/health.test.ts")).toBe(false);
  ...
  it("keeps runtime paths restart-relevant", () => {
    expect(shouldTrackDevServerPath("apps/server/src/routes/health.ts")).toBe(true);
    expect(shouldTrackDevServerPath("packages/shared/src/index.ts")).toBe(true);
    expect(shouldTrackDevServerPath("apps/server/src/testing/runtime.ts")).toBe(true);
  });
});
