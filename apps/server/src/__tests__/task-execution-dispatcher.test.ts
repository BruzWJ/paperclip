...
type AttemptSettlement = Parameters<
  TaskExecutionAttemptExecutor["execute"]
>[2];

async function settleResult<T extends Awaited<
  ReturnType<TaskExecutionAttemptExecutor["execute"]>
>>(settle: AttemptSettlement, result: T): Promise<T> {
  await settle(result);
  return result;
}

function ref(change: Partial<TaskExecutionRef> = {}): TaskExecutionRef {
  return {
    id: "ref",
    companyId: "company",
    taskId: "task",
...
describe("task execution dispatcher", () => {
  ...
  it("cannot apply a stale attempt signal to a later attempt in the same Session", async () => {
    ...
    const secondLease: LeasedTaskExecutionRef = {
      ref: secondRef,
