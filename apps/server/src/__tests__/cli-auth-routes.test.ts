import * as t from "./cli-auth-routes.test-support.js";
const { describe, registerSuiteSetup, it, mockBoardAuthService, CHALLENGE_ID } = t;
const { createApp, request, expect, testBoardSessionActor, mockLogActivity } = t;
const { SECOND_CHALLENGE_ID, testBoardKeyActor } = t;
...
describe.sequential("cli auth routes", () => {
  ...
  it.sequential(
    ...
    async () => {
      ...
      expect(res.body.approvalUrl).toContain(`/cli-auth/${CHALLENGE_ID}?token=pcp_cli_auth_secret`);
    },
    15_000,
  );

  it.sequential("rejects noncanonical CLI challenge text and unknown fields", async () => {
    const app = await createApp({ type: "none", source: "none" });
    for (const body of [
      ...
      const res = await request(app).post("/api/cli-auth/challenges").send(body);
      expect(res.status, res.text || JSON.stringify(res.body)).toBe(400);
    }
    expect(mockBoardAuthService.createCliAuthChallenge).not.toHaveBeenCalled();
  });
  ...
  it.sequential("marks challenge status as requiring sign-in for anonymous viewers", async () => {
    ...
    const app = await createApp({ type: "none", source: "none" });
    const res = await request(app).get(`/api/cli-auth/challenges/${CHALLENGE_ID}?token=pcp_cli_auth_secret`);
  ...
  it.sequential("returns access only for the exact authenticated user ID", async () => {
    mockBoardAuthService.resolveBoardAccess.mockResolvedValue({
      user: { id: "user-1", name: "User One", email: "user@example.com" },
      companyIds: ["company-1"],
      memberships: [],
  ...
  it.sequential("rejects board key label, timestamp, and payload aliases", async () => {
    const app = await createApp(
      testBoardKeyActor({
        userId: "user-1",
        isInstanceAdmin: false,
  ...
  it.sequential("lists and revokes named board API keys for the current board user", async () => {
    ...
    expect(mockBoardAuthService.listBoardApiKeys).toHaveBeenCalledWith("user-1", { includeInactive: false });
