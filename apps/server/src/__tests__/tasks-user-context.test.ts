...
describe("deriveTaskUserContext", () => {
  ...
  it("uses task creation time as fallback touch point for creator", () => {
    const context = deriveTaskUserContext(
      makeTask({ creatorUserId: "user-1", createdAt: new Date("2026-03-06T09:00:00.000Z") }),
      "user-1",
      {
  ...
  it("uses task updated time as fallback touch point for owner", () => {
    const context = deriveTaskUserContext(
      makeTask({ ownerUserId: "user-1", updatedAt: new Date("2026-03-06T15:00:00.000Z") }),
      "user-1",
      {
        myLastCommentAt: null,
        myLastReadAt: null,
