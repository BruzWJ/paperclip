...
describe("secret provider registry", () => {
  ...
  it("describes managed and external-reference provider capabilities", () => {
    const descriptors = createSecretProviderRegistry(
  ...
  it("warns when the local encrypted key file is readable by group or others", async () => {
    const dir = path.join(
      os.tmpdir(),
      `paperclip-secret-provider-${randomBytes(6).toString("hex")}`,
    );
    tmpDirs.push(dir);
    ...
    const checks = await createSecretProviderRegistry(
      testSecretsRuntimeConfig({ masterKeyFilePath: keyFile }),
    ).check();
