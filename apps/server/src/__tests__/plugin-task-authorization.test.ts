...
} from "../services/plugin-task-authorization.js";
...
function canonicalInvokableCatalog() {
  const agents: InvokableTaskOwnerAgent[] = [
    {
...
function resolve(
  overrides: Partial<
    Parameters<typeof resolvePluginPermittedTaskOwnerCatalog>[0]
  > = {},
) {
...
describe("plugin task owner authorization", () => {
  it("intersects approved create permission with canonical company-wide invokability and current revisions", () => {
    ...
    expect(() =>
      selectPluginPermittedTaskOwner(catalog, {
        companyId: COMPANY_ID,
        pluginInstallationId: INSTALLATION_ID,
        pluginKey: PLUGIN_KEY,
        operation: "tasks.create",
  ...
  it("uses the persisted manifest capability for the exact operation", () => {
    expect(() => resolve({ operation: "tasks.update" })).toThrowError(
      expect.objectContaining({
        code: "plugin_task_authorization_rejected",
  ...
  it.each([
    ...
    try {
      ...
      expect(error).toBeInstanceOf(PluginTaskAuthorizationRejected);
  ...
  it("uses the shared installation-company-setting prefix for company setting mutations", async () => {
    ...
    const db = {
      transaction<T>(
        callback: (transaction: TaskSessionDbTransaction) => Promise<T>,
      ) {
        return callback(tx);
      },
    } as unknown as Db;
  ...
  it("locks and rechecks the complete requested-owner catalog in one transaction", async () => {
    ...
    await expect(
      assertPluginPermittedTaskOwnerInTransaction(tx, {
        companyId: COMPANY_ID,
        pluginInstallationId: INSTALLATION_ID,
        pluginKey: PLUGIN_KEY,
        operation: "tasks.create",
