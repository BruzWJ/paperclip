...
import { OrdinaryTaskRuntimeRejected } from "../services/ordinary-task-runtime.js";
...
const firstTaskId = "00000000-0000-4000-8000-000000000020";
const secondTaskId = "00000000-0000-4000-8000-000000000021";

const createTask = vi.fn();
const ordinaryTasks = { create: createTask } as never;
...
function createdResult(input: {
  ...
  return {
    ...
    ref: { id: `ordinary-task-create:${companyId}:${input.idempotencyKey}` },
...
function createApp(db: Db) {
  ...
  app.use("/api", taskRoutes(db, {} as never, { ordinaryTasks }));
  app.use(errorHandler);
  return app;
}

describe("task create deduplication routes", () => {
  ...
  it("returns 201 for the canonical create and 200 for an exact idempotent replay", async () => {
    ...
    const first = await request(app).post(`/api/companies/${companyId}/tasks`).send(input);
    const replay = await request(app).post(`/api/companies/${companyId}/tasks`).send(input);

    expect(first.status).toBe(201);
    expect(replay.status).toBe(200);
    expect(first.body).toMatchObject({ id: firstTaskId, request: input.request, ownerAgentId, retried: false });
    expect(replay.body).toMatchObject({ id: firstTaskId, request: input.request, ownerAgentId, retried: true });
  ...
  it("maps an immutable-payload idempotency rejection to the canonical conflict response", async () => {
    ...
    createTask
      .mockResolvedValueOnce(createdResult({ ...input, retried: false }))
      .mockRejectedValueOnce(new OrdinaryTaskRuntimeRejected(
    ...
    const first = await request(app).post(`/api/companies/${companyId}/tasks`).send(input);
    const conflict = await request(app).post(`/api/companies/${companyId}/tasks`).send({
      ...input,
    ...
    expect(conflict.body).toMatchObject({
      error: "Task creation idempotency key was retried with different immutable input",
  ...
  it("keeps identical titles independent when the explicit idempotency keys differ", async () => {
    ...
    const first = await request(app).post(`/api/companies/${companyId}/tasks`).send({
      request: "First independent request.",
      ownerAgentId,
      title: "Coordinate launch",
      idempotencyKey: "run-2:coordinate-launch-a",
    });
    const second = await request(app).post(`/api/companies/${companyId}/tasks`).send({
      request: "Second independent request.",
      ownerAgentId,
      title: "Coordinate launch",
      idempotencyKey: "run-2:coordinate-launch-b",
    ...
    expect(first.body.id).toBe(firstTaskId);
    expect(second.body.id).toBe(secondTaskId);
  });
  ...
  it("preserves serialized retry outcomes returned by the canonical runtime", async () => {
    ...
    const [first, retry] = await Promise.all([
      request(app).post(`/api/companies/${companyId}/tasks`).send(input),
      request(app).post(`/api/companies/${companyId}/tasks`).send(input),
    ]);
  ...
  it("passes exact request bytes and canonical board creator identity to the runtime", async () => {
    ...
    createTask.mockResolvedValue(createdResult({ ...input, retried: false }));
    const harness = createMockDb();
