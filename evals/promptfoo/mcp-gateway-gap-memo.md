...
- Agent response while a gateway-created action request is pending (`409 approval_required`): wait on the board decision instead of re-executing the write.
...
- Agent target resolution for selected concrete company tools: use the exact `mcp.<application-connection>:<tool>` target rather than an ambiguous upstream tool name or similar connection.
