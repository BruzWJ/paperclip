...
// Browser suites exercise the real Vite UI against the test-owned API fixture
// in fixtures.ts. They never boot Paperclip or inherit a database target.
...
export default defineConfig({
  testDir: ".",
  testMatch: "**/*.spec.ts",
  testIgnore: ["multi-user-authenticated.spec.ts"],
  timeout: 60_000,
  retries: 0,
  workers: 1,
  use: {
