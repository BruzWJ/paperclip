...
describe("pipeline conversation canonical cutover", () => {
  it("has exactly one production writer for conversation correlations", () => {
    const routeSource = readSource("server/src/routes/pipelines.ts");
    const serviceSource = readSource("server/src/services/pipelines.ts");
  ...
  it("keeps case-body documents board-side without an issue-document annotation mirror", () => {
    ...
    const componentSource = readSource(
      "ui/src/components/PipelineItemBodyDocument.tsx",
    );
    ...
    expect(
      existsSync(
        path.join(
          repoRoot,
          "server/src/services/pipeline-conversation-context.ts",
        ),
      ),
    ).toBe(false);
  });
