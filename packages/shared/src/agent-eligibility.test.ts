...
describe("agent work eligibility", () => {
  ...
  it("blocks idle descendants of terminated ancestors and reports repair details", () => {
    ...
    expect(health.fullChain).toEqual([
      ...
      expect.objectContaining({ id: "manager-2", name: "Manager 2", status: "terminated", relation: "ancestor", depth: 1 }),
      expect.objectContaining({ id: "director-2", name: "Director 2", status: "terminated", relation: "ancestor", depth: 2 }),
    ]);
    expect(health.firstInvalidAncestor).toEqual({ id: "manager-2", name: "Manager 2", status: "terminated" });
    expect(health.invalidAncestors).toEqual([
      { id: "manager-2", name: "Manager 2", status: "terminated" },
      { id: "director-2", name: "Director 2", status: "terminated" },
    ]);
    expect(health.repairGuidance).toContain("QA 2 reports through terminated ancestor Manager 2");
  ...
  it("blocks agents with reporting cycles", () => {
    ...
    expect(health.fullChain).toEqual([
      ...
      expect.objectContaining({ id: "manager-4", relation: "ancestor", depth: 1 }),
