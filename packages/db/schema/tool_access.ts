...
export const connectionGrants = pgTable(
  "connection_grants",
  {
    ...
    revokedByUserId: text("revoked_by_user_id").references(() => authUsers.id, {
      onDelete: "restrict",
    }),
...
export const toolConnectionInstalls = pgTable(
  ...
  (table) => [
    check(
      "tool_connection_installs_target_shape_check",
      sql`(
        ${table.targetType} = 'company'
        and ${table.targetAgentId} is null
      ) or (
        ${table.targetType} = 'agent'
    ...
    foreignKey({
      columns: [table.companyId, table.connectionId],
      foreignColumns: [toolConnections.companyId, toolConnections.id],
      name: "tool_connection_installs_company_connection_fk",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.companyId, table.targetAgentId],
      foreignColumns: [agents.companyId, agents.id],
      name: "tool_connection_installs_company_target_agent_fk",
    }).onDelete("cascade"),
    index("tool_connection_installs_company_target_idx").on(
      table.companyId,
      table.targetType,
      table.targetAgentId,
    ),
    index("tool_connection_installs_connection_idx").on(table.companyId, table.connectionId),
    uniqueIndex("tool_connection_installs_company_target_uq")
      .on(table.companyId, table.connectionId)
      .where(sql`${table.targetType} = 'company'`),
    uniqueIndex("tool_connection_installs_agent_target_uq")
      .on(table.companyId, table.connectionId, table.targetAgentId)
      .where(sql`${table.targetType} = 'agent'`),
    unique("tool_connection_installs_selection_identity_uq").on(
      table.companyId,
      table.connectionId,
      table.id,
      table.targetAgentId,
...
export const toolCatalogEntries = pgTable(
  "tool_catalog_entries",
  {
    ...
    reviewedByUserId: text("reviewed_by_user_id").references(() => authUsers.id, {
      onDelete: "set null",
    }),
  ...
  (table) => [
    ...
    unique("tool_catalog_entries_company_connection_id_uq").on(
      table.companyId,
      table.connectionId,
      table.id,
    ),
...
export const agentCompanyToolSelections = pgTable(
  "agent_company_tool_selections",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull(),
    agentId: uuid("agent_id").notNull(),
    connectionId: uuid("connection_id").notNull(),
    connectionInstallId: uuid("connection_install_id").notNull(),
    catalogEntryId: uuid("catalog_entry_id").notNull(),
    catalogVersionHash: text("catalog_version_hash").notNull(),
    status: text("status")
      .$type<"selected" | "revoked">()
      .notNull()
      .default("selected"),
    selectedByKind: text("selected_by_kind")
      .$type<"user" | "agent" | "plugin" | "migration">()
      .notNull(),
    selectedByAgentId: uuid("selected_by_agent_id").references(() => agents.id, {
      onDelete: "set null",
    }),
    selectedByUserId: text("selected_by_user_id").references(() => authUsers.id, {
      onDelete: "restrict",
    }),
  ...
  (table) => [
    check(
      "agent_company_tool_selections_status_check",
      sql`${table.status} in ('selected', 'revoked')`,
    ),
    check(
      "agent_company_tool_selections_selected_actor_check",
      sql`(
        ${table.selectedByKind} = 'user'
        and ${table.selectedByUserId} is not null
        and ${table.selectedByAgentId} is null
        and ${table.selectedByPluginInstallationId} is null
      ) or (
        ${table.selectedByKind} = 'agent'
        and ${table.selectedByAgentId} is not null
        and ${table.selectedByUserId} is null
        and ${table.selectedByPluginInstallationId} is null
      ) or (
        ${table.selectedByKind} = 'plugin'
        and ${table.selectedByPluginInstallationId} is not null
        and ${table.selectedByAgentId} is null
        and ${table.selectedByUserId} is null
      ) or (
        ${table.selectedByKind} = 'migration'
        and ${table.selectedByAgentId} is null
        and ${table.selectedByUserId} is null
        and ${table.selectedByPluginInstallationId} is null
      )`,
    ...
    check(
      "agent_company_tool_selections_revocation_check",
      sql`(
        ${table.status} = 'selected'
        and ${table.revokedByKind} is null
        and ${table.revokedByAgentId} is null
        and ${table.revokedByUserId} is null
        and ${table.revokedByPluginInstallationId} is null
        and ${table.revocationReason} is null
        and ${table.revokedAt} is null
      ) or (
        ${table.status} = 'revoked'
        and ${table.revokedByKind} is not null
        and ${table.revocationReason} is not null
        and ${table.revokedAt} is not null
        and (
          (
            ${table.revokedByKind} = 'user'
            and ${table.revokedByUserId} is not null
            and ${table.revokedByAgentId} is null
            and ${table.revokedByPluginInstallationId} is null
          ) or (
            ${table.revokedByKind} = 'agent'
            and ${table.revokedByAgentId} is not null
            and ${table.revokedByUserId} is null
            and ${table.revokedByPluginInstallationId} is null
          ) or (
            ${table.revokedByKind} = 'plugin'
            and ${table.revokedByPluginInstallationId} is not null
            and ${table.revokedByAgentId} is null
            and ${table.revokedByUserId} is null
          ) or (
            ${table.revokedByKind} = 'migration'
            and ${table.revokedByAgentId} is null
            and ${table.revokedByUserId} is null
            and ${table.revokedByPluginInstallationId} is null
          )
    ...
    foreignKey({
      ...
      foreignColumns: [agents.companyId, agents.id],
      name: "agent_company_tool_selections_company_agent_fk",
    }).onDelete("cascade"),
    foreignKey({
      columns: [
        table.companyId,
        table.connectionId,
        table.connectionInstallId,
        table.agentId,
      ],
      foreignColumns: [
      ...
      name: "agent_company_tool_selections_exact_agent_install_fk",
    }).onDelete("restrict"),
    foreignKey({
      columns: [table.companyId, table.connectionId, table.catalogEntryId],
      foreignColumns: [
        toolCatalogEntries.companyId,
        toolCatalogEntries.connectionId,
        toolCatalogEntries.id,
      ],
      name: "agent_company_tool_selections_catalog_entry_fk",
    }).onDelete("restrict"),
    unique("agent_company_tool_selections_company_id_uq").on(
      table.companyId,
      table.id,
    ),
    uniqueIndex("agent_company_tool_selections_active_uq")
      .on(table.companyId, table.agentId, table.catalogEntryId)
      .where(sql`${table.status} = 'selected'`),
    index("agent_company_tool_selections_agent_status_idx").on(
      table.companyId,
      table.agentId,
      table.status,
    ),
    index("agent_company_tool_selections_install_idx").on(
      table.companyId,
      table.connectionInstallId,
    ),
  ],
...
export const toolMcpGatewayTokens = pgTable(
  "tool_mcp_gateway_tokens",
  {
    ...
    expiryOverrideByUserId: text("expiry_override_by_user_id").references(() => authUsers.id, {
      onDelete: "restrict",
    }),
  ...
  (table) => [
    check(
      "tool_mcp_gateway_tokens_subject_type_check",
      sql`${table.subjectType} in ('gateway_client', 'board_user', 'agent')`,
    ),
...
export const toolInvocations = pgTable(
  "tool_invocations",
  {
    ...
    agentId: uuid("agent_id").references(() => agents.id, { onDelete: "set null" }),
    issueId: uuid("issue_id").references(() => issues.id, { onDelete: "set null" }),
    runId: uuid("run_id").references(() => issueExecutionRuns.id, { onDelete: "set null" }),
    ...
    runInterfaceToolCallId: uuid("run_interface_tool_call_id"),
    callIdentitySource: text("call_identity_source").$type<
      "provider" | "jsonrpc"
    >(),
    callIdentityType: text("call_identity_type").$type<
      "string" | "number"
    >(),
    callIdentityValue: text("call_identity_value"),
    applicationId: uuid("application_id").references(() => toolApplications.id, { onDelete: "set null" }),
    connectionId: uuid("connection_id").references(() => toolConnections.id, { onDelete: "set null" }),
    connectionInstallId: uuid("connection_install_id").references(
      () => toolConnectionInstalls.id,
      { onDelete: "set null" },
    ),
  ...
  (table) => [
    check(
      "tool_invocations_call_identity_check",
      sql`(
        ${table.callIdentitySource} is null
        and ${table.callIdentityType} is null
        and ${table.callIdentityValue} is null
      ) or (
        ${table.callIdentitySource} in ('provider', 'jsonrpc')
        and ${table.callIdentityType} in ('string', 'number')
        and ${table.callIdentityValue} is not null
      )`,
    ),
    ...
    index("tool_invocations_run_interface_call_idx").on(
      table.companyId,
...
export const toolActionRequests = pgTable(
  "tool_action_requests",
  {
    ...
    canonicalArguments: jsonb("canonical_arguments").$type<unknown>().notNull(),
    ...
    dispatchIdempotencyKey: text("dispatch_idempotency_key"),
  ...
  (table) => [
    ...
    uniqueIndex("tool_action_requests_company_dispatch_uq").on(
      table.companyId,
      table.dispatchIdempotencyKey,
    ),
  ],
);
