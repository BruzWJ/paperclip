...
export const routines = pgTable(
  "routines",
  {
    ...
    activityGatePolicy: text("activity_gate_policy")
      .notNull()
      .default("always"),
    ...
    responsibleUserId: text("responsible_user_id").references(
      () => authUsers.id,
      {
        onDelete: "set null",
      },
  ...
  (table) => ({
    companyStatusIdx: index("routines_company_status_idx").on(
      table.companyId,
      table.status,
    ),
    companyAssigneeIdx: index("routines_company_assignee_idx").on(
      table.companyId,
      table.assigneeAgentId,
    ),
    companyProjectIdx: index("routines_company_project_idx").on(
      table.companyId,
      table.projectId,
    ),
    companyFolderIdx: index("routines_company_folder_idx").on(
      table.companyId,
      table.folderId,
    ),
    companyResponsibleUserIdx: index(
      "routines_company_responsible_user_idx",
    ).on(table.companyId, table.responsibleUserId),
    companyOriginIdx: index("routines_company_origin_idx").on(
      table.companyId,
      table.originKind,
      table.originId,
    ),
...
export const routineRevisions = pgTable(
  ...
  (table) => ({
    ...
    companyRoutineCreatedIdx: index(
      "routine_revisions_company_routine_created_idx",
    ).on(table.companyId, table.routineId, table.createdAt),
    companyResponsibleUserIdx: index(
      "routine_revisions_company_responsible_user_idx",
    ).on(table.companyId, table.responsibleUserId, table.createdAt),
  }),
);
...
export const routineTriggers = pgTable(
  ...
  (table) => ({
    companyRoutineIdx: index("routine_triggers_company_routine_idx").on(
      table.companyId,
      table.routineId,
    ),
    companyKindIdx: index("routine_triggers_company_kind_idx").on(
      table.companyId,
      table.kind,
    ),
...
export const routineRuns = pgTable(
  "routine_runs",
  {
    ...
    source: text("source").$type<RoutineRunSource>().notNull(),
    status: text("status")
    ...
    triggeredAt: timestamp("triggered_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
    routineRevisionId: uuid("routine_revision_id").references(
      () => routineRevisions.id,
      { onDelete: "set null" },
    ),
    responsibleUserId: text("responsible_user_id").references(
  ...
  (table) => ({
    sourceCheck: check(
      "routine_runs_source_check",
      sql`${table.source} in ('schedule', 'manual', 'api', 'webhook')`,
    ),
    statusCheck: check(
      "routine_runs_status_check",
      sql`${table.status} in ('received', 'coalesced', 'skipped', 'task_created', 'completed', 'failed')`,
    ),
    companyRoutineIdx: index("routine_runs_company_routine_idx").on(
      table.companyId,
      table.routineId,
      table.createdAt,
    ),
    routineRevisionIdx: index("routine_runs_revision_idx").on(
      table.routineRevisionId,
    ),
    companyResponsibleUserIdx: index(
      "routine_runs_company_responsible_user_idx",
    ).on(table.companyId, table.responsibleUserId, table.createdAt),
    triggerIdx: index("routine_runs_trigger_idx").on(
      table.triggerId,
      table.createdAt,
    ),
    dispatchFingerprintIdx: index("routine_runs_dispatch_fingerprint_idx").on(
      table.routineId,
      table.dispatchFingerprint,
    ),
    linkedTaskIdx: index("routine_runs_linked_task_idx").on(table.linkedTaskId),
    idempotencyIdx: index("routine_runs_trigger_idempotency_idx").on(
      table.triggerId,
