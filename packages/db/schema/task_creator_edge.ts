...
import { taskExecutionAuthorities } from "./task_execution_runtime.js";
import { tasks } from "./tasks.js";
import { taskSessions } from "./task_sessions.js";

// Immutable creator provenance plus the single canonical task-update ledger.
export const taskCreatorEdgeReceivability = pgTable(
  "task_creator_edge_receivability",
  {
  ...
  (table) => [
    ...
    check(
      "task_creator_edge_receivability_terminal_reason_check",
    ...
    foreignKey({
      ...
      name: "task_creator_edge_receivability_scope_fk",
    }).onDelete("cascade"),
    unique("task_creator_edge_receivability_scope_id_uq").on(
      table.companyId,
      table.taskId,
      table.ownershipEpoch,
      table.id,
    ),
    unique("task_creator_edge_receivability_admission_identity_uq").on(
      table.companyId,
      table.taskId,
      table.ownershipEpoch,
      table.id,
    ...
    unique("task_creator_edge_receivability_epoch_uq").on(
      table.companyId,
      table.taskId,
      table.ownershipEpoch,
    ),
    index("task_creator_edge_receivability_current_idx").on(
      table.companyId,
      table.taskId,
      table.state,
    ),
    index("task_creator_edge_receivability_endpoint_idx").on(
      table.companyId,
      table.endpointKind,
      table.endpointId,
      table.state,
...
export const taskUpdates = pgTable(
  "task_updates",
  {
    ...
    status: text("status").$type<AgentVisibleTaskStatus>(),
  ...
  (table) => [
    ...
    check("task_updates_run_sequence_check", sql`${table.runSequence} >= 0`),
    check(
      "task_updates_creator_edge_check",
    ...
    foreignKey({
      ...
      name: "task_updates_source_authority_fk",
    }).onDelete("restrict"),
    unique("task_updates_scope_id_uq").on(
      table.companyId,
      table.taskId,
      table.ownershipEpoch,
      table.id,
    ),
    uniqueIndex("task_updates_gateway_invocation_uq").on(
      table.companyId,
      table.gatewayInvocationId,
    ),
    uniqueIndex("task_updates_run_sequence_uq").on(
      table.companyId,
      table.runId,
      table.runSequence,
    ),
    uniqueIndex("task_updates_comment_uq").on(table.commentId),
    index("task_updates_task_sequence_idx").on(
      table.companyId,
      table.taskId,
      table.ownershipEpoch,
      table.createdAt,
...
export const pluginWithdrawalOperations = pgTable(
  "plugin_withdrawal_operations",
  {
    ...
    taskUpdateId: uuid("task_update_id").references(() => taskUpdates.id, {
      onDelete: "restrict",
    }),
    mutationCommentId: uuid("mutation_comment_id").references(() => taskComments.id, {
      onDelete: "restrict",
    }),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  ...
  (table) => [
    ...
    index("plugin_withdrawal_operations_task_idx").on(
      table.companyId,
      table.taskId,
      table.createdAt,
    ),
...
export const systemEscalationIdentities = pgTable(
  "system_escalation_identities",
  {
    ...
    companyId: uuid("company_id")
      .notNull()
      .references(() => companies.id, { onDelete: "cascade" }),
    affectedTaskId: uuid("affected_task_id")
      .notNull()
      .references(() => tasks.id, { onDelete: "restrict" }),
    affectedOwnershipEpoch: integer("affected_ownership_epoch").notNull(),
    escalationTaskId: uuid("escalation_task_id")
      .notNull()
      .references(() => tasks.id, { onDelete: "restrict" }),
    systemSource: text("system_source")
      .$type<SystemCreatorSourceKind>()
      .notNull(),
    triggeringRunId: uuid("triggering_run_id").references(() => taskExecutionRuns.id, {
      onDelete: "restrict",
    }),
    terminalCreatorEdgeId: uuid("terminal_creator_edge_id")
      .notNull()
      .references(() => taskCreatorEdgeReceivability.id, {
        onDelete: "restrict",
      }),
  ...
  (table) => [
    ...
    check(
      "system_escalation_identities_distinct_task_check",
      sql`${table.affectedTaskId} <> ${table.escalationTaskId}`,
    ),
    uniqueIndex("system_escalation_identities_affected_epoch_uq").on(
      table.companyId,
      table.affectedTaskId,
      table.affectedOwnershipEpoch,
    ),
    uniqueIndex("system_escalation_identities_escalation_task_uq").on(
      table.companyId,
      table.escalationTaskId,
    ),
    unique(
