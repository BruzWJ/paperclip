...
export const executionWorkspaces = pgTable(
  "execution_workspaces",
  {
    ...
    workspaceClass: text("workspace_class")
      .$type<"project" | "projectless">()
      .notNull()
      .default("project"),
  ...
  (table) => ({
    ...
    workspaceClassCheck: check(
      "execution_workspaces_class_check",
      sql`(
        ${table.workspaceClass} = 'project'
        and ${table.projectId} is not null
      ) or (
        ${table.workspaceClass} = 'projectless'
        and ${table.projectId} is null
        and ${table.projectWorkspaceId} is null
        and ${table.cwd} is not null
        and left(${table.cwd}, 1) = '/'
      )`,
    ),
  }),
);
