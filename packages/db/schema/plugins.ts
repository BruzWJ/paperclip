 ...
 *
 * Each live plugin installation is uniquely identified by `plugin_key`
 * (derived from the manifest `id`). Uninstalled rows are immutable
 * installation tombstones, so reinstalling the same key creates a new row
 * and a new installation identity. The full manifest is persisted as JSONB in
...
export const plugins = pgTable(
  ...
  (table) => ({
    pluginKeyIdx: uniqueIndex("plugins_plugin_key_idx")
      .on(table.pluginKey)
