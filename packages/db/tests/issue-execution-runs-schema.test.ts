...
describe("canonical issue execution run schema", () => {
  it("keeps the run envelope closed and free of prompt or result mirrors", () => {
    const config = getTableConfig(issueExecutionRuns);
    const columns = columnNames(issueExecutionRuns);

    expect(config.name).toBe("issue_execution_runs");
    ...
    expect(config.foreignKeys.map((key) => key.getName())).toEqual(
      expect.arrayContaining([
        "issue_execution_runs_lane_fk",
        "issue_execution_runs_current_attempt_id_issue_execution_attempts_id_fk",
        "issue_execution_runs_current_lease_id_issue_execution_leases_id_fk",
  ...
  it("owns immutable ordered membership and the closed settlement matrix", () => {
    const config = getTableConfig(issueExecutionRunRefs);
    const uniqueNames = config.uniqueConstraints.map((constraint) =>
      constraint.getName(),
    );
    const activeRef = config.indexes.find(
      (candidate) =>
        candidate.config.name === "issue_execution_run_refs_active_ref_uq",
    );

    expect(config.name).toBe("issue_execution_run_refs");
    expect(uniqueNames).toEqual(
      expect.arrayContaining([
        "issue_execution_run_refs_run_ordinal_uq",
        "issue_execution_run_refs_run_ref_uq",
  ...
  it("reserves segment zero for the run-ref and exposes the trigger candidate key", () => {
    const config = getTableConfig(issueExecutionPromptSegments);

    expect(config.name).toBe("issue_execution_prompt_segments");
    expect(
      checkSql(
        issueExecutionPromptSegments,
        "issue_execution_prompt_segments_positive_ordinal_check",
      ),
    ).toContain('"issue_execution_prompt_segments"."segment_ordinal" > 0');
    expect(
    ...
    expect(columnNames(issueExecutionPromptSegments)).toEqual(
      expect.arrayContaining([
        "source_message_id",
        "source_input_id",
    ...
    expect(columnNames(issueExecutionPromptSegments)).not.toContain("input_id");
    expect(config.foreignKeys.map((key) => key.getName())).toEqual(
      expect.arrayContaining([
        "issue_execution_prompt_segments_source_message_fk",
        "issue_execution_prompt_segments_source_input_fk",
        "issue_execution_prompt_segments_terminal_message_fk",
      ]),
    );
    expect(
      checkSql(
    ...
    ).toContain('"source_input_id" = "issue_execution_prompt_segments"."source_message_id"');
    expect(
      checkSql(
        issueExecutionPromptSegments,
  ...
  it("keeps the current-prompt control row to exactly four structural columns", () => {
    const config = getTableConfig(issueExecutionRunControls);

    expect(config.name).toBe("issue_execution_run_controls");
    expect(columnNames(issueExecutionRunControls)).toEqual([
      "run_id",
      "current_ref_id",
      "current_ordinal",
      "current_segment_ordinal",
  ...
  it("owns one exact same-target FIFO lane and leaves causal refs immutable", () => {
    const lane = getTableConfig(issueExecutionLanes);
    const refs = getTableConfig(issueExecutionRefs);

    expect(lane.name).toBe("issue_execution_lanes");
    expect(columnNames(issueExecutionLanes)).toEqual([
      "company_id",
      "issue_id",
      "ownership_epoch",
      "target_agent_id",
    ...
    expect(
      checkSql(issueExecutionLanes, "issue_execution_lanes_active_lease_check"),
    ).toContain('"active_lease_generation" > 0');
    expect(
      checkSql(issueExecutionLanes, "issue_execution_lanes_active_lease_check"),
    ).toContain('"active_lease_id" is not null');
    expect(
      checkSql(issueExecutionLanes, "issue_execution_lanes_ordinal_check"),
    ).toContain("9007199254740991");
    expect(lane.indexes.map((index) => index.config.name)).toContain(
      "issue_execution_lanes_active_lease_uq",
    );

    expect(columnNames(issueExecutionRefs)).toContain("lane_ordinal");
    expect(columnNames(issueExecutionRefs)).not.toEqual(
      expect.arrayContaining([
        "run_id",
        "lease_state",
        "lease_id",
    ...
    expect(
      checkSql(issueExecutionRefs, "issue_execution_refs_lane_ordinal_check"),
    ).toContain("9007199254740991");
  });
  ...
  it("binds one typed attempt to a base or steering prompt", () => {
    ...
    expect(config.name).toBe("issue_execution_attempts");
    expect(columnNames(issueExecutionAttempts)).toEqual([
      "id",
      "company_id",
      "issue_id",
      "session_id",
    ...
    expect(identity).toContain(
      '"steering_segment_ordinal" = "issue_execution_attempts"."segment_ordinal"',
    );
    expect(
      columnNames(issueExecutionAttempts).filter(
  ...
  it("owns delayed pre-send retries outside the closed run envelope", () => {
    const config = getTableConfig(issueExecutionAttemptRetrySchedules);

    expect(config.name).toBe("issue_execution_attempt_retry_schedules");
    expect(columnNames(issueExecutionAttemptRetrySchedules)).toEqual([
      "id",
      "company_id",
      "issue_id",
      "run_id",
    ...
    expect(config.foreignKeys.map((key) => key.getName())).toEqual(
      expect.arrayContaining([
        "issue_execution_attempt_retry_schedules_predecessor_fk",
        "issue_execution_attempt_retry_schedules_successor_fk",
      ]),
    );
    expect(config.uniqueConstraints.map((key) => key.getName())).toEqual(
      expect.arrayContaining([
        "issue_execution_attempt_retry_schedules_predecessor_uq",
        "issue_execution_attempt_retry_schedules_successor_uq",
      ]),
    );
    ...
    ).toContain('"retry_at" >= "issue_execution_attempt_retry_schedules"."created_at"');
  ...
  it("owns one lease and one process group for each prompt attempt", () => {
    ...
    expect(leases.foreignKeys.map((key) => key.getName())).toContain(
      "issue_execution_leases_attempt_fk",
    );
  ...
  it("owns cancellation only through the exact typed attempt", () => {
    const config = getTableConfig(issueExecutionCancellationIntents);
    const columns = columnNames(issueExecutionCancellationIntents);

    expect(config.name).toBe("issue_execution_cancellation_intents");
    expect(columns).toEqual([
      "id",
      "company_id",
      "issue_id",
    ...
    expect(config.uniqueConstraints.map((key) => key.getName())).toContain(
      "issue_execution_cancellation_intents_attempt_uq",
    );
  });

  it("keeps liveness to the exact identity and five-field immutable fact", () => {
    expect(columnNames(issueExecutionRunLivenessFacts)).toEqual([
      "id",
      "company_id",
      "run_id",
      "liveness_state",
    ...
    expect(
      getTableConfig(issueExecutionRunLivenessFacts).uniqueConstraints.map(
        (constraint) => constraint.getName(),
      ),
    ).toContain("issue_execution_run_liveness_facts_run_uq");
  });

  it("keeps finalization reference-only and text-free", () => {
    const columns = columnNames(issueExecutionFinalizations);

    expect(getTableConfig(issueExecutionFinalizations).name).toBe(
