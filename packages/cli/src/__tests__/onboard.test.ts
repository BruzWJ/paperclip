...
const TEST_DATABASE_URL =
  "postgresql://paperclip.invalid/paperclip_onboard_test";
let isolatedHome = "";
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "paperclip-onboard-fresh-"),
  );
...
describe("onboard", () => {
  beforeEach(() => {
    ...
    delete process.env.PAPERCLIP_STORAGE_S3_FORCE_PATH_STYLE;
    ...
    delete process.env.SERVE_UI;
    delete process.env.BETTER_AUTH_SECRET;
    isolatedHome = fs.mkdtempSync(
      path.join(os.tmpdir(), "paperclip-onboard-test-home-"),
    );
  ...
  it("keeps --yes onboarding on private loopback defaults", async () => {
    ...
    expect(raw.server).not.toHaveProperty("host");
  ...
  it("creates instance-root config and data paths for a fresh PAPERCLIP_HOME", async () => {
    const home = fs.mkdtempSync(
      path.join(os.tmpdir(), "paperclip-onboard-home-"),
    );
    const cwd = fs.mkdtempSync(
      path.join(os.tmpdir(), "paperclip-onboard-cwd-"),
    );
    process.chdir(cwd);
    ...
    expect(fs.readFileSync(envPath, "utf8")).toMatch(
      /^BETTER_AUTH_SECRET=[A-Za-z0-9_-]{43}$/m,
    );
  ...
  it("fails closed when tailnet quickstart cannot resolve a Tailscale address", async () => {
    const configPath = createFreshConfigPath();
    delete process.env.PAPERCLIP_TAILNET_BIND_HOST;
    process.env.PATH = "";
  ...
  it.each([
    ["PAPERCLIP_BIND_HOST", ""],
    ["PAPERCLIP_BIND_HOST", " 192.0.2.10"],
    ["PAPERCLIP_TAILNET_BIND_HOST", ""],
    ["PAPERCLIP_TAILNET_BIND_HOST", "100.64.0.8 "],
    ["PAPERCLIP_PUBLIC_URL", ""],
    ["PAPERCLIP_STORAGE_LOCAL_DIR", " /tmp/paperclip"],
    ["PAPERCLIP_SECRETS_MASTER_KEY_FILE", "/tmp/key "],
    ["DATABASE_URL", " postgres://database.invalid/paperclip"],
  ])("rejects non-canonical %s=%j without falling back", async (key, value) => {
    const configPath = createFreshConfigPath();
    ...
    await expect(
      onboard({
        ...
        yes: true,
        invokedByRun: true,
        ...(key === "PAPERCLIP_TAILNET_BIND_HOST" ? { bind: "tailnet" } : {}),
      }),
  ...
  it.each([
    ...
    ["PAPERCLIP_STORAGE_PROVIDER", "filesystem"],
    ["PAPERCLIP_SECRETS_PROVIDER", "LOCAL_ENCRYPTED"],
    ["PAPERCLIP_STORAGE_S3_FORCE_PATH_STYLE", "1"],
    ["PAPERCLIP_SECRETS_STRICT_MODE", "yes"],
    ["SERVE_UI", "TRUE"],
    ["PORT", "03100"],
  ])("rejects non-canonical %s=%j", async (key, value) => {
    const configPath = createFreshConfigPath();
    process.env[key] = value;
