...
function CostRows({
  ...
  return (
    <Card>
      ...
      <CardContent>
        {rows.length === 0 ? (
          <div className="text-sm text-muted-foreground">No settled prompts in this range.</div>
        ) : (
          <div className="divide-y divide-border">
            {rows.map((row) => (
              <div key={row.id} className="grid gap-2 py-3 sm:grid-cols-(--gtc-17) sm:items-center">
                <div className="min-w-0">
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {formatNumber(row.eventCount)} finance event{row.eventCount === 1 ? "" : "s"}; currencies are never mixed.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-4 sm:grid-cols-2">
        <div>
          <div className="text-xs text-muted-foreground">Debits</div>
          <div className="mt-1 font-mono text-sm">{formatMoneyAmount(row.debitAmount, row.currency)}</div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">Credits</div>
          <div className="mt-1 font-mono text-sm">{formatMoneyAmount(row.creditAmount, row.currency)}</div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">Net {row.netDirection}</div>
          <div className="mt-1 font-mono text-sm">{formatMoneyAmount(row.netAmount, row.currency)}</div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">Estimated debits</div>
          <div className="mt-1 font-mono text-sm">{formatMoneyAmount(row.estimatedDebitAmount, row.currency)}</div>
        </div>
      </CardContent>
    </Card>
  );
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={CircleDollarSign} message="Select a company to view costs." />;
  }
  if (costQuery.isLoading || budgetQuery.isLoading || financeQuery.isLoading) {
    return <PageSkeleton variant="dashboard" />;
  }
  if (costQuery.error || budgetQuery.error || financeQuery.error || !costQuery.data || !budgetQuery.data || !financeQuery.data) {
    return <EmptyState icon={TriangleAlert} message="Cost and budget data could not be loaded." />;
  }
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="budgets" className="mt-5 space-y-5">
          {budgetData.activeIncidents.map((incident) => (
            <BudgetIncidentCard
              ...
              onKeepPaused={() => incidentMutation.mutate({
                incidentId: incident.id,
                action: "keep_paused",
              })}
              onRaiseAndResume={(limitAmount) => incidentMutation.mutate({
                incidentId: incident.id,
        ...
        <TabsContent value="events" className="mt-5">
          <Card>
            ...
            <CardContent>
              {events.length === 0 ? (
                <div className="text-sm text-muted-foreground">No settled prompt facts in this range.</div>
              ) : (
                <div className="divide-y divide-border">
                  {events.map((event) => (
                    <div key={event.id} className="grid gap-2 py-3 sm:grid-cols-(--gtc-17) sm:items-center">
                      <div>
                        <div className="text-sm font-medium">{event.kind === "known" ? "Known cost" : "Cost unavailable"}</div>
                        <div className="text-xs text-muted-foreground">
                      ...
                      <div className="font-mono text-sm sm:text-right">
                        {event.kind === "known" && event.knownDeltaAmount
                          ...
                          : event.unavailableReason?.replaceAll("_", " ") ?? "unavailable"}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <TabsContent value="finance" className="mt-5 space-y-5">
          {financeData.summary.currencies.length === 0 ? (
            <Card className="p-5 text-sm text-muted-foreground">No finance events in this range.</Card>
          ) : (
            <div className="grid gap-5 xl:grid-cols-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {financeData.byBiller.map((row) => (
                  <div key={`${row.biller}:${row.currency}`} className="flex items-center justify-between gap-4 text-sm">
                    <span>{row.biller} · {row.currency}</span>
                    <span className="font-mono">{formatMoneyAmount(row.netAmount, row.currency)}</span>
                  </div>
                ))}
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-3">
                {financeData.byKind.map((row) => (
                  <div key={`${row.eventKind}:${row.currency}`} className="flex items-center justify-between gap-4 text-sm">
                    <span>{row.eventKind.replaceAll("_", " ")} · {row.currency}</span>
                    <span className="font-mono">{formatMoneyAmount(row.netAmount, row.currency)}</span>
                  </div>
                ))}
              </CardContent>
            ...
            <CardContent className="divide-y divide-border">
              {financeData.events.map((event) => (
                <div key={event.id} className="flex items-center justify-between gap-4 py-3 text-sm">
                  <div>
                    <div className="font-medium">{event.description ?? event.eventKind.replaceAll("_", " ")}</div>
                    <div className="text-xs text-muted-foreground">{event.biller} · {formatDateTime(event.occurredAt)}</div>
                  </div>
                  <div className="font-mono">{formatMoneyAmount(event.amount, event.currency)}</div>
                </div>
              ))}
            </CardContent>
          </Card>
