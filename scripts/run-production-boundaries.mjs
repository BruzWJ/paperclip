...
export const PRODUCTION_BOUNDARY_GATE_STEPS = Object.freeze([
  ...
  nodeScript(
    "check:task-comment-projector-writers",
    "scripts/check-task-comment-projector-writers.mjs",
  ),
  tsxScript("check:task-vocabulary", "scripts/check-task-vocabulary.ts"),
  nodeScript("check:paperclip-session-structure", "scripts/check-paperclip-session-structure.mjs"),
  tsxScript(
    "check:task-session-durable-writers",
    "scripts/check-task-session-durable-writers.ts",
  ),
  nodeScript("check:canonical-task-writers", "scripts/check-canonical-task-writers.mjs"),
  tsxScript(
    "check:task-execution-run-service-boundary",
    "scripts/check-task-execution-run-service-boundary.ts",
  ),
  tsxScript("check:cross-task-memory-removal", "scripts/check-cross-task-memory-removal.ts"),
...
export const PRODUCTION_BOUNDARY_SELF_TEST_STEPS = Object.freeze([
  ...
  nodeTest(
    ...
    "scripts/check-task-comment-projector-writers.test.mjs",
  ),
  tsxTest("test:task-vocabulary", "scripts/check-task-vocabulary.test.ts"),
  tsxTest("test:task-session-durable-writers", "scripts/check-task-session-durable-writers.test.ts"),
  tsxTest("test:canonical-human-auth", "scripts/check-canonical-human-auth.test.ts"),
  nodeTest("test:canonical-task-writers", "scripts/check-canonical-task-writers.test.mjs"),
  tsxTest(
    "test:task-execution-run-service-boundary",
    "scripts/check-task-execution-run-service-boundary.test.ts",
  ),
  tsxTest("test:cross-task-memory-removal", "scripts/check-cross-task-memory-removal.test.ts"),
