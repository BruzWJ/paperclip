...
test("rejects a retired Paperclip child environment channel", () => {
  ...
  write(
    ...
    `const injected = ${JSON.stringify(retired)};\n`,
  );
  assert.ok(
...
test("rejects a retired instruction materializer", () => {
  ...
  assert.ok(
    providerIdentityBoundaryViolations(root).some((entry) =>
      entry.includes("retired instruction owner"),
    ),
  );
});
...
test("rejects an explicit-environment prefix ban", () => {
  ...
  write(
    root,
    "packages/shared/src/validators/agent.ts",
    'if (key.startsWith("PAPERCLIP_")) reject();\n',
  );
  assert.ok(
    providerIdentityBoundaryViolations(root).some((entry) =>
      entry.includes("prefix-banned"),
    ),
  );
});
...
test("rejects opaque provider string inspection", () => {
  ...
  write(
    root,
    "packages/shared/src/validators/agent.ts",
    "const FORBIDDEN_ADAPTER_STRING_PATTERN = /bridge/;\n",
  );
  assert.ok(
    providerIdentityBoundaryViolations(root).some((entry) =>
      entry.includes("inspected"),
    ),
  );
});
...
test("rejects a Paperclip-authored setup prompt override", () => {
  ...
  assert.ok(
    providerIdentityBoundaryViolations(root).some((entry) =>
      entry.includes("prompt override"),
    ),
  );
});
